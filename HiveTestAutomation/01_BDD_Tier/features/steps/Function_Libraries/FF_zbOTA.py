...
deviceTypes = [{'type': 'SLR1', 'ep': '05', 'path': 'SLR1_SLR2', 'zbType': 'FFD'},
               ...
               {'type': 'SLB2', 'ep': '09', 'path': 'SLB2', 'zbType': 'FFD'},
               ...
               {'type': 'FWBulb01', 'ep': '01', 'path': 'Aurora_FWBulb01', 'zbType': 'FFD'},
               {'type': 'FWBulb01US', 'ep': '01', 'path': 'Aurora_FWBulb01US', 'zbType': 'FFD'},
               {'type': 'FWPAR38Bulb01US', 'ep': '01', 'path': 'Aurora_FWPAR38Bulb01US', 'zbType': 'FFD'},
               {'type': 'FWGU10Bulb01UK', 'ep': '01', 'path': 'Aurora_FWGU10Bulb01UK', 'zbType': 'FFD'},
               {'type': 'FWCLBulb01UK', 'ep': '01', 'path': 'Aurora_FWCLBulb01UK', 'zbType': 'FFD'},
               {'type': 'TWGU10Bulb01UK', 'ep': '01', 'path': 'Aurora_TWGU10Bulb01UK', 'zbType': 'FFD'},
               {'type': 'TWCLBulb01UK', 'ep': '01', 'path': 'Aurora_TWCLBulb01UK', 'zbType': 'FFD'},
               ...
               {'type': 'SLB2c', 'ep': '09', 'path': 'SLB2c', 'zbType': 'FFD'},
               {'type': 'SLP2d', 'ep': '09', 'path': 'SLP2d', 'zbType': 'FFD'}
               ]
...
def sedPostUpgradeChecks(result, header, nodeId, ep, timeout=30 * 60):
    ...
    while doloop:
        ...
        strModel = utils.getModelforDevice(nodeId, ep)

        oDictSED = AT.oDictSED

        if result.postUpgradeQniVersion is None and str(strModel).upper() not in oDictSED:
            if not msg.startswith(qniMsg):
                print('\n waiting for QNI \n')
                payload = AT.waitForQNI(nodeId, ep, timeout=15 * 60)
                if payload['currentVersion'] == header.fileVersion:
                    result.postUpgradeQni = time.time()
                    result.postUpgradeQniVersion = payload['currentVersion']

        if msg.startswith(qniMsg) and str(strModel).upper() not in oDictSED:
            version = msg.split(',')[5]
        ...
        if msg.startswith(ciMsg):
            ...
            if not respState:
                ...
                if respValue == header.fileVersion:
                    ...
                    print('Error : ' + respValue + " -- " + header.fileVersion)
        if str(strModel).upper() not in oDictSED:
            if result.finalVersion == header.fileVersion and result.postUpgradeQniVersion == header.fileVersion:
                doloop = False
        else:
            if result.finalVersion == header.fileVersion:
                result.postUpgradeQniVersion = result.finalVersion
                doloop = False
...
def ffdPostUpgradeChecks(result, header, nodeId, ep, timeout=700):
    ...
    while doLoop:
        if state == 'checkImageNotify':
            # _Send two imageNotify msgs to check QNI is generated
            for _ in range(2):
...
def firmwareUpgrade(nodeId, ep, zbType, imageFile, header, printData=PRINT_DATA):
    ...
    if zbType == 'SED':
        ...
        # longPollInt, checkInInt = AT.setCompletFastPoll(nodeId,ep)
    ...
    while testInProgress:
        ...
        if state == 'waitQniOrCheckIn':
            ...
            if not respState:
                ...
                if printData: pbar = tqdm(total=int(header.totalImageSize, 16))
                state = 'waitIBRequest'
            ...
            if not respState:
                ...
                print(str(int(header.totalImageSize, 16)) + " ---- " + str(int(payloadBytes, 16)) + "\n")
                if printData and int(offset, 16)> int(str(progressOffset), 16):
                    progressOffset=offset
                    pbar.update(int(payloadBytes,16))
                state = 'waitIBRequest'
            ...
            if zbType == 'SED':
                sedPostUpgradeChecks(result, header, nodeId, ep)
                state = 'exit'
            else:
                ffdPostUpgradeChecks(result, header, nodeId, ep)
                state = 'exit'
...
if __name__ == "__main__":
    ...
    AT.startSerialThreads(config.PORT, config.BAUD, printStatus=AT.debug, rxQ=True, listenerQ=False)
    # Start the upgrades
    result = firmwareUpgrade(nodeId, ep, zbType, imageFile, header, printData=PRINT_DATA)
