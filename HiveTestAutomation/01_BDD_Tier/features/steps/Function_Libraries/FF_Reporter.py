...
class Reporter:
    ...
    def __init__(self):
        ...
        self.currentcaseid = {}
        self.testrailurl = ""
        self.testrailusername = ""
        self.testrailpassword = ""
        self.istestrunupdate = ""
    ...
    def HTML_Execution_Summary_Initialize(self, strCurrentApplication = "Hive", strCurrentEnvironment = "Beta_Env"):
        ...
        try:
            ...
            oFileWriter.write("< meta http - equiv = 'refresh' content = '5' >\n")
            strChartData = ""
            if str(utils.getAttribute("COMMON", "apiValidationType", None, None)).upper() == "ZIGBEE API":
                try:
                    dutils.putZigbeeDevicesJson(dutils.getNodes(True))
    ...
    def ReportEvent(self, strValidationType, strLogDescription, strStatus, strTableAlign = 'LEFT', boolLogwithTimeStamp = True, driver = None,ocrImagePath = None):
        try:
            ...
            if not driver is None:
                ...
                if ocrImagePath is not None:
                    strScreenShotPathHyperlink = '.' + os.sep + '..' + os.sep + 'Screenshot' + os.sep + ocrImagePath
                    oFileWriter.write("<td class='" + strStatus.lower() + "'><a href='" + strScreenShotPathHyperlink + "'>" + strStatus + "</a></td>\n")
                else:
            ...
            if "FAIL" in strStatus.upper():
                ...
                testrunupdate = testRailConnect()
                if self.istestrunupdate == "true":
                        testrunupdate.updatetestrun(5, self.runid, self.currentcaseid, self.testrailurl,
                                                    self.testrailusername, self.testrailpassword)
    ...
    def HTML_TestCase_Footer(self):
        ...
        try:
            ...
            testrunupdate = testRailConnect()
            if self.strTCStatus == "PASSED":
                self.intPassTCCount += 1
                if self.istestrunupdate == "true":
                  testrunupdate.updatetestrun(1,self.runid,self.currentcaseid,self.testrailurl,self.testrailusername,self.testrailpassword)
            if self.strTCStatus == "FAILED":
                self.intFailTCCount += 1
                if self.istestrunupdate == "true":
                  testrunupdate.updatetestrun(5,self.runid,self.currentcaseid,self.testrailurl,self.testrailusername,self.testrailpassword)
        except:
    ...
    def HTML_Execution_Summary_TCAddLink(self):
        ...
        try:
            ...
            if (self.strTCStatus == "PASSED"): 
             self.intPassTCCount += 1
             qry = 'add_result_for_case/' + str(self.runid) + '/' + str(self.currentcaseid)
              testupdateresult= self.testrailapiconnect.send_post(qry,
               {'status_id': statusid})
             Update_TC_Status()
            if (self.strTCStatus == "FAILED"): 
             self.intFailTCCount += 1
    ...
    def logCreation(self,actualDict,expectedDict):

        actualAttributes =  list(actualDict.values())[0]
        expectedAttributes = list(expectedDict.values())[0]
        boolStatus = True
        ...
        for eachAttribute in actualAttributes.keys():
            ...
            if actualAttributeValue == expectedAttributeValue:
                actualAttributes[eachAttribute] = '$$'+actualAttributeValue
            else:
                actualAttributes[eachAttribute] = '$$||' + actualAttributeValue
                boolStatus = False

        if boolStatus:
            strHeader = 'Attributes$$' + list(expectedDict.keys())[0] + ' values and ' + list(actualDict.keys())[0] + ' values@@@'
            for eachAttribute in actualAttributes.keys():
                actualAttributes[eachAttribute] = ''
            strStatus = "PASS"
        else:
            strHeader = 'Attributes$$'+list(expectedDict.keys())[0] +' values$$'+list(actualDict.keys())[0]+' values@@@'
            strStatus = "FAIL"
        resultstr = strHeader
