...
extraRxQueue = queue.Queue(maxsize=1000)

stopThread = threading.Event()
threadPool = []
ser = None

oDictSED = ['MOT003', 'DWS003', 'PIR00140005', 'WDS00140002', 'SLT3', 'SLT3B', 'SLT2', "BUTTON01",'TMP001']
        ...
        if reading != '':
            ...
            if rxQ:
                if rxQueue.full():
                    ...
                    rxQueue.get()
                if extraRxQueue.full():
                    extraRxQueue.get()
                rxQueue.put(reading)
                extraRxQueue.put(reading)
            ...
            if debug:
                print("DEBUG RX: {},  {}".format(myTime, reading))
                if UIgui is not None:
                    lbl = UIgui.Label(
                        "DEBUG RX: {},  {}".format(myTime, reading),
                        width='100%')
                    UIObj.consoleContainer.append(lbl)
                    UIObj.consoleContainer.redraw()
...
def startSerialThreadsReturn(port, baud, printStatus=False, rxQ=False, listenerQ=False):
    """
        rxQ and listenerQ are flags to switch on/off the appropriate Q
        defaults are off.
...
def flushRxQ():
    ...
    while not rxQueue.empty():
        rxQueue.get()
    while not extraRxQueue.empty():
        extraRxQueue.get()
    return


def sendCommand(cmd, myExpectedResponses, maxAttempts=5, retryTimeout=30, UI=False):
    ...
    myTime = datetime.datetime.now().strftime("%H:%M:%S.%f")
    if debug:
        print("DEBUG Tx: {},  {},Expected Response={}".format(myTime, cmd, myExpectedResponses))
        if UIgui is not None:
            lbl = UIgui.Label("DEBUG Tx: {},  {},Expected Response={}".format(myTime, cmd, myExpectedResponses), width='100%')
    ...
    while doLoop:
        ...
        if not rxQueue.empty():
            resp = rxQueue.get()
            respStateNack, respCodeNack = matchResponse(resp, ['NACK:(..)'])
            if not respStateNack:
            ...
            if resp.startswith('DFTREP') or resp.startswith('DFTMREP'):
                r = resp.split(',')
                if r[4] == '82':
                    respState = False
                    respCode = '82'
                ...
                if r[4] == 'C3':
                    respState = False
                    respCode = 'C3'
                    respValue = 'Reserved Command'
                    return respState, respCode, respValue
                if r[4] == '84':
                    respState = False
                    respCode = '84'
                    respValue = 'ZCL_UNSUPPORTED_MANUFACTURER_GENERAL_COMMAND'
                    return respState, respCode, respValue
        ...
        if time.time() > lastTry + retryTimeout:
            if tryCount < maxAttempts:
                ...
                myTime = datetime.datetime.now().strftime("%H:%M:%S.%f")
                if debug:
                    print("DEBUG Tx: {},  {}".format(myTime, myMessage))
                    if UIgui is not None:
                        lbl = UIgui.Label(
                            "DEBUG Tx: {},  {}".format(myTime, myMessage),
                            width='100%')
                        lbl.style['text-align'] = 'right'
                        UIObj.consoleContainer.append(lbl)
                        UIObj.consoleContainer.redraw()
        ...
        try:
            ...
            if respVal.startswith(('TEMPERATURE', 'REPORTATTR', 'REPORTMATTR', 'CWSCHEDULE', 'RESPATTR', 'RESPMATTR')):
                ...
                if respVal.startswith('TEMPERATURE'):
                    ...
                    clustId = '0201'
                    attrId = "sched{}".format(
                        resp[3])  # In this case attrId is actually "sched" + the day of week bitmap
                    attrVal = ''.join(resp[5:])
...
def discEndpoints(myNodeId):
    ...
    respState, respCode, respValue = sendCommand(myMsg, expectedResponses, 30)
...
def discClusters(myNodeId, myEP):
    ...
    if respState and respCode == zcl.statusCodes['SUCCESS']:
        ...
        myList = [rxQueue.get(), rxQueue.get()]
...
def discAttrs_old(myNodeId, myEP, myClusterId, myClusterType, myStart='0000', numAttrsToRetrieve='FF'):
    ...
    # DISCATTR:D198,05,00
    """ Discover standard attrs and manufacturer specific attrs """
...
def discAttrs(myNodeId, myEP, myClusterId, myClusterType, myStart='0000', numAttrsToRetrieve='FF',mspAttr=False):
    """ Returns all the attributes for the given cluster
    ...
    """ Discover standard attrs and then manufacturer specific attrs """

    # Discover the standard attrs
    startAttr = myStart
    attrCount = numAttrsToRetrieve
    while startAttr:
        if not mspAttr:
            msg, expectedResponse = buildDiscAttrs(myNodeId, myEP, myClusterId, startAttr, attrCount, myClusterType,
                                                   mspAttr)
            respState, respCode, respValue = sendCommand(msg, expectedResponse)
            if respState:
                startAttr = parseDiscAttrsResponse(respValue, respData)
            else:
                respData = respValue
                break
        if mspAttr:
            # Discover any manufacturer specific attributes
            ...
            msg, expectedResponse = buildDiscAttrs(myNodeId, myEP, myClusterId, startAttr, attrCount, myClusterType,
                                                   mspAttr)
            respState, respCode, respValue = sendCommand(msg, expectedResponse,maxAttempts=1,retryTimeout=1)
            if respState:
                startAttr = parseDiscAttrsResponse(respValue, respData)
            else:
                respData = respValue
...
def buildDiscAttrs(myNodeId, myEP, myClusterId, startAttr, attrCount, clustType, mspAttr=False):
    """  Builds a discover attribute command for client/server clusters and standard or manufacturer specific attributes

        Uses rawzcl commands
    ...
    fcServerManSpecific = '04'
    fcClientManSpecific = '0C'

    if mspAttr:
        if clustType == 'server':
            fc = fcServerManSpecific
        else:
            fc = fcClientManSpecific
        expectedResponse = ['DISCMATTR:{0},{1},{2},(..)'.format(myNodeId, myEP, MANUFACTURER_ID)]
        manId = MANUFACTURER_ID
    ...
    return writeMsg, expectedResponse


def parseDiscAttrsResponse(respValue, respData):
    """ Parses the response and returns:
            - Index for next chunk if not all found in this pass.  Index=None if all found.
            - Append found attributes into the respData list
    ...
    rType = temp[0].split(':')[0]
    if rType == 'DISCATTR':
        mspAttr = False
    elif rType == 'DISCMATTR':
        mspAttr = True
    else:
        print("ERROR: Unexpected response in parseDiscAttrsResponse")
        exit()
    ...
    for i in range(0, len(respTempList)):
        ...
        attrId = temp[1][5:]
        attrType = temp[2][5:]
        respData.append((attrId, attrType, mspAttr))
        if int(attrId, 16) > int(lastAttr, 16):
            lastAttr = attrId
...
def getAttribute(myNodeId, myEP, myClust, myAttr, myClusterType, mspAttr=False):
    """ Returns the attribute value and the respstate
        myClusterType should be 'client' or 'server'

    """
    myMsg, expectedResponse = buildGetAttributeCommands(myNodeId, myEP, myClust, myAttr, myClusterType, mspAttr)
    respState, respCode, respValue = sendCommand(myMsg, expectedResponse)
...
def buildGetAttributeCommands(myNodeId, myEP, myClust, myAttr, myClusterType):
    """
    """
    sendMode = '0'
    if myClust in zcl.BG_Clusters:
        if myClusterType == 'server':
            myMsg = 'AT+READMATR:{0},{1},{2},{3},{4},{5}'.format(myNodeId, myEP, sendMode, MANUFACTURER_ID, myClust,
                                                                 myAttr)
        else:
            myMsg = 'AT+READMCATR:{0},{1},{2},{3},{4},{5}'.format(myNodeId, myEP, sendMode, MANUFACTURER_ID, myClust,
                                                                  myAttr)
        ...
        # ILLUMINANCE:2E6F,06,0000,00,FFFF

        if myClust == '0402':
            expectedResponse = ['TEMPERATURE:{0},{1},{2},(..)'.format(myNodeId, myEP, myAttr)]
        elif myClust == '0400':
            expectedResponse = ['ILLUMINANCE:{0},{1},{2},(..)'.format(myNodeId, myEP, myAttr)]
        else:
            expectedResponse = ['RESPATTR:{0},{1},{2},{3},(..)'.format(myNodeId, myEP, myClust, myAttr)]

    return myMsg, expectedResponse


def buildGetAttributeCommands(myNodeId, myEP, myClust, myAttr, myClusterType, mspAttr=False):
    """
    """
    sendMode = '0'
    ...
    # Deterine the expected response
    if mspAttr:
        expectedResponse = ['RESPMATTR:{0},{1},{2},{3},{4},(..)'.format(myNodeId,
                                                                        myEP,
                                                                        MANUFACTURER_ID,
...
def setChekinAttribute(myNodeId, myEP, myClust, myClusterType, myAttr, myAttrType, myAttrVal):
    """
    """
    sendMode = '0'
    ...
    if myClust in zcl.BG_Clusters:
        myMsg = 'AT+WRITEMATR:{0},{1},{2},{3},{4},{5},{6},{7}'.format(myNodeId, myEP, sendMode, MANUFACTURER_ID,
                                                                      myClust, myAttr, myAttrType, myAttrVal)
        expectedResponse = ['WRITEMATTR:{0},{1},{2},{3},,(..)'.format(myNodeId, myEP, MANUFACTURER_ID, myClust)]
    else:
        myMsg = 'AT+WRITEATR:{0},{1},{2},{3},{4},{5},{6}'.format(myNodeId, myEP, sendMode, myClust, myAttr, myAttrType,
                                                                 myAttrVal)
        expectedResponse = ['WRITEATTR:{0},{1},{2},(..)'.format(myNodeId, myEP, myClust)]
...
def setAttribute(myNodeId, myEP, myClust, myClusterType, myAttr, myAttrType, myAttrVal):
    ...
    # Only currently support writing of sever type attributes
    assert myClusterType == 'server'
    if '_msp' in str(myAttr):
        myAttr = str(myAttr).replace('_msp', '')
    if myClust in zcl.BG_Clusters:
        myMsg = 'AT+WRITEMATR:{0},{1},{2},{3},{4},{5},{6},{7}'.format(myNodeId, myEP, sendMode, MANUFACTURER_ID,
                                                                      myClust, myAttr, myAttrType, myAttrVal)
        expectedResponse = ['WRITEMATTR:{0},{1},{2},{3},,(..)'.format(myNodeId, myEP, MANUFACTURER_ID, myClust)]
...
def getAttributeReporting(myNodeId, myEpId, myClustId, myDirection, myAttr, mspAttr):
    """ Get the reporting intervals (if any) set for the given attribute

            This always looks for attributes that are reported NOT received.
    ...
    # Reporting direction
    attrReported = '00'  # 0x00=Attribute reported, 0x01 = Attribute received

    zclCmd = '08'  # Read reporting configuration
    ...
    if mspAttr:
        ...
        expectedResponse = ['READRPTCFGRSP:{0},{1},{2},(..),{3},{4}'.format(myNodeId,
                                                                            ...
                                                                            myAttr)]
        manId = ''
    myMsg = "AT+RAWZCL:{0},{1},{2},{3}{4}{5}{6}{7}{8}".format(myNodeId, myEpId, myClustId, fc, manId, seqNum, zclCmd,
                                                              repDir, byteSwap(myAttr))
...
def setBinding(myNodeId, mySrcAddr, mySrcEp, myCluster, myDstAddr, myDstEp, UI=False):
    """
        AT+BIND:<address>,<type>,<SrcAddress>,<SrcEP>,<ClusterID>,<DstAddress>
    """
    myType = '3'  # 3=Unicast
    msg = 'AT+BIND:{0},{1},{2},{3},{4},{5},{6}'.format(myNodeId, myType, mySrcAddr, mySrcEp, myCluster, myDstAddr,
                                                       myDstEp)
...
def setFastPoll():
    ...
    respState, _, _ = sendCommand(fpStartMsg, expectedResponses)
    if not respState:
        return False
    return True
...
def gpGet(myNodeId, myEpId, mySendMode, myCount='00', myGpList=None):
    ...
    # GETGPRSP:<NodeID>,<EP>,<Capacity>, <Count>,<GPID>_
...
def sceneAdd(myNodeId, myEpId, mySendMode, myGroupId, mySceneId, myTransitionTime, mySceneName, myExtFieldSets):
    """ Add a scene to a group on a single device.
        Group address is not supported
    ...
    # _We join these together into a single string

    # AT+SCADD:<Address>,<EP>,<SendMode>, <GPID>,<SCID>,<TransTime>,<SCName>, [Extension field sets]
    myExtFieldSets = ','.join(myExtFieldSets)
    msg = "AT+SCADD:{0},{1},{2},{3},{4},{5},{6},{7}".format(myNodeId, myEpId, mySendMode, myGroupId, mySceneId,
                                                            myTransitionTime, mySceneName, myExtFieldSets)

    # ADDSC:<NodeID>,<EP>,<Status>[,<GPID>, <SCID>]
    if mySendMode == '0':
        expectedResponses = ['ADDSCRSP:{0},{1},(..)'.format(myNodeId, myEpId)]
    else:
        print("ERROR: sceneAdd does not support group addressing.")

    # Send the command
    respState, respCode, respValue = sendCommand(msg, expectedResponses)
    if respState and respCode == zcl.statusCodes['SUCCESS'] and mySendMode == 0:
        respValue = respValue.split(',')[3:]
...
def sceneView(myNodeId, myEpId, mySendMode, myGroupId, mySceneId):
    ...
    """
    VIEWSCRSP:<NodeID>,<EP>,<Status>,[<GPID>,<SCID>,<SCName>]#
              ...
              |    |  |  |    |  |    |  ExtensionFeildSets
              v    v  v  v    v  v    v /-----------------\
    VIEWSCRSP:081A,01,00,0001,01,000E,,0006,01,01,0008,01,80

    """
    expectedResponses = ['VIEWSCRSP:{0},{1},(..),{2},{3}'.format(myNodeId, myEpId, myGroupId, mySceneId)]
    # Send the command
    respState, respCode, respValue = sendCommand(msg, expectedResponses)
    if respState and respCode == zcl.statusCodes['SUCCESS']:
        respValue = respValue.split(',')[7:]
...
def sceneStore(myNodeId, myEpId, mySendMode, myGroupId, mySceneId):
    ...
    msg = "AT+SCSTORE:{0},{1},{2},{3},{4}".format(myNodeId, myEpId, mySendMode, myGroupId, mySceneId)
    ...
    # Send the command
    respState, respCode, respValue = sendCommand(msg, expectedResponses)
    if respState and mySendMode == '0' and respCode == zcl.statusCodes['SUCCESS']:
        respValue = respValue.split(',')[3:]

    return respState, respCode, respValue


def sceneRecall(myNodeId, myEpId, mySendMode, myGroupId, mySceneId):
    """ Recall (apply) the given scene attribute values.

    """
    # AT+SCRECAL:<Address>,<EP>,<SendMode>,<GPID>,<SCID>
    msg = "AT+SCRECAL:{0},{1},{2},{3},{4}".format(myNodeId, myEpId, mySendMode, myGroupId, mySceneId)
...
def sceneGetMembership(myNodeId, myEpId, mySendMode, myGroupId):
    """ List all the scenes on a device for the given group

    """
    # AT+SCGETMSH:<Address>,<EP>, <SendMode>,<GPID>
    msg = 'AT+SCGETMSH:{0},{1},{2},{3}'.format(myNodeId, myEpId, mySendMode, myGroupId)

    # SCMSHRSP:<NodeID>,<EP>,<Status>, [<Capacity>,<GPID>,<SCCount>,<SCList>]
    expectedResponses = ['SCMSHRSP:{0},{1},(..)'.format(myNodeId, myEpId)]

    # Send the command
    respState, respCode, respValue = sendCommand(msg, expectedResponses)
    if respState and mySendMode == '0':
        capacity = respValue.split(',')[3]
        groupId = respValue.split(',')[4]
        sceneCount = respValue.split(',')[5]
        sceneList = respValue.split(',')[6:]
        respValue = {'capacity': capacity, 'groupId': groupId, 'sceneCount': sceneCount, 'sceneList': sceneList}
...
def moveToLevel(myNodeId, myEpId, mySendMode='0', myLevel=100, myDuration=0):
    """
    ...
    assert myLevel >= 0
    assert myLevel <= 100
    myLevel = int(myLevel / 100 * 255)

    myMsg = 'AT+LCMVTOLEV:{},{},{},{},{:02x},{:04x}'.format(myNodeId, myEpId, mySendMode, onOff, myLevel, myDuration)
...
def colourTemperature(myNodeId, myEp, mySendMode='0', myColourTemp=4000, myDuration=0):
    ...
    if mySendMode == '0':
        expectedResponses = ['DFTREP:{},{},0300,0A,(..)'.format(myNodeId, myEp)]
    else:
        expectedResponses = ['OK']
    respState, respCode, respValue = sendCommand(myMsg, expectedResponses)
    return respState, respCode, respValue
...
def pjoin(duration):
    """
    """
    print("Network Permit request broadcasted")
    myMsg = "AT+PJOIN:{:02x}".format(duration)
    #myMsg = "AT+PJOIN:"
    expectedResponse = ['OK']
    respState, respCode, respValue = sendCommand(myMsg, expectedResponse)
    return respState, respCode, respValue
...
def setMode(myNodeId, myEp, myMode, myEpType, mySetpointFloat=None, myDuration=1):
    ...
    if myMode == 'OFF':
        ...
        if myEpType == 'HEAT' and mySetpointFloat is None:
        ...
        if myEpType == 'HEAT':
            attrs = [('systemMode', '05'), ('temperatureSetpointHold', '01'),
                     ('temperatureSetpointHoldDuration', durationHex),
...
def checkValidSetpoint(mySetpoint):
    ...
    if mySetpoint == 1 or (5 <= mySetpoint <= 32):
        return True, None
    else:
...
def imageNotify(myNodeId, myEp, myManufCode, mySendMode='0', myPayloadType='03',
    ...
    """ Send imageNotify message
        ...
        timeout = 700             #_More than 10mins to allow all devices a chance to
...
def upgradeEndResponse(myNodeId, myEp, myManufCode, myImageType, myFileVersion, seqNum, currentTime=None,
                       upgradeTime=None):
...
def waitForImageBlockReqOrUpgradeEndReq(myNodeId, myEp, timeout=120):
    ...
    if msg is None:
        ...
        elif resp[2] == '01' or resp[2] == '02':
            payload['seqNum'] = resp[9]
        else:
...
def waitForQNI(myNodeId, myEp, timeout=700):
    ...
    if msg is None:
        ...
        payload = parseQNI(msg)
        payload['error'] = None
    return payload


def waitForCheckIn(myNodeId, myEp, timeout=700):
    """ checkIn is typically sent every 5mins, so we'll wait for longer than that.
        Returns True if checkIn received, esle False
    """
    resp = waitForMessage(['CHECKIN:{},{}'.format(myNodeId, myEp)], timeout=timeout)
...
def parseQNI(msg):
    """ Parse the QNI response and return a dict with
        manufCode
        imageType
        currentVersion
        hardwareVersion
    ...
    qniResp = msg.split(',')
    payload = {'manufCode': qniResp[3], 'imageType': qniResp[4], 'currentVersion': qniResp[5]}
...
def getInitialData(nodeId, fastPoll=True, printStatus=False):
    ...
    params['pollControlEp'] = None
    for ep in endpoints:
        # Get the simple descriptor & find the ep that has poll control cluster
            ...
            for f in ['ProfileID', 'DeviceID', 'InCluster', 'OutCluster']:
                if f == 'DeviceID':
                    ...
                    if deviceType is None:
                        print(
                            "{:10} = {}. ****** DEVICE TYPE NOT LISTED IN ZIGBEE CLUSTER LIBRARY.  PLEASE ADD IT. (Page 49 HA Spec)".format(
                                f, respValue[f]))
                    else:
        ...
        pollControlCluster = '0020'
        if "InCluster" in respValue:
            if pollControlCluster in respValue['InCluster']:
                params['pollControlEp'] = ep
                print("Poll Control EP = {}".format(ep))
    ...
    if nodeType == 'ZED' and fastPoll:
        ...
        # _Set a new shorter long poll interval
        myInterval = 5  # 5s
        ...
        myInterval = 30
        myIntervalHex = "{:08X}".format(myInterval * 4)
        respState, respCode, respValue = setAttribute(nodeId, srcEp, pollControlClustId, 'server', checkInId,
                                                      checkInType, myIntervalHex)
        if not respState:
        ...
        resp = setFastPoll()
        if not resp:
            print("Fast poll set failed.")
            exit()
        # Wait for a checkIn so that device receives fast poll start
        fpStarted = waitForCheckIn(nodeId, params['pollControlEp'])
        if not fpStarted:
...
def getAllAttributes(myNodeId):
    ...
    for ep in endpoints:
        ...
        for clust in clusterList:
            ...
            clustType = clust[0]
            if debug: print("\nDEBUG: Starting on {}".format(clustName))
            respState, respCode, respValue = discAttrs(myNodeId, ep, clustId, clustType,mspAttr=False)
            ...
            if respState and respCode == zcl.statusCodes['SUCCESS']:
                for attr in respValue:
                    ...
                    mspAttr = attr[2]
                    if not mspAttr: printAttrValuesAndReporting(myNodeId, ep, clustId, attrId, attrType, clustType,
                                                                mspAttr)

            else:
                print("Problem with attribute discovery: ", respValue)
                exit()
            respState, respCode, respValue = discAttrs(myNodeId, ep, clustId, clustType, mspAttr=True)
            print("\nEndpoint={0}, Cluster={1},{2},{3},Manufacturer Specific Attributes".format(ep, clust[1], clustName,
                                                                                                clustType))
            if respState and respCode == zcl.statusCodes['SUCCESS']:
                for attr in respValue:
                    attrId = attr[0]
                    attrType = attr[1]
                    mspAttr = attr[2]
                    if mspAttr: printAttrValuesAndReporting(myNodeId, ep, clustId, attrId, attrType, clustType, mspAttr)
            else:
                print("Problem with attribute discovery: ", respValue)
                #exit()
...
def printAttrValuesAndReporting(myNodeId, ep, clustId, attrId, attrType, clustType, mspAttr):
    """ Get the attribute data and reporting configuration and print it nicely
    """
    ...
    respState, respCode, respVal = getAttribute(myNodeId, ep, clustId, attrId, clustType, mspAttr)
    if respState and respCode == zcl.statusCodes['SUCCESS']:
        attrVal = respVal

        # Get the reporting intervals for the attribute
        respState, respCode, attrReport = getAttributeReporting(myNodeId, ep, clustId, clustType, attrId, mspAttr)
        if respState and respCode == zcl.statusCodes['SUCCESS']:
            print("{0},{1},{2:32},{3:20},{4}".format(attrId, attrType, zclAttrName, attrVal, attrReport))
            if attrType != zclAttrType and zclAttrName != "*** Unknown Attr. ***": print(
            ...
            if respCode == zcl.statusCodes['ZCL_NOT_FOUND']:
                ...
                print('{0},{1},{2:32},{3:20},ZCL_UNREPORTABLE_ATTRIBUTE'.format(attrId,
                                                                                ...
                                                                                zclAttrName,
                                                                                attrVal))
            elif respCode == zcl.statusCodes['ZCL_UNSUPPORTED_ATTRIBUTE']:
                print('{0},{1},{2:32},{3:20},ZCL_UNSUPPORTED_ATTRIBUTE'.format(attrId,
                                                                               attrType,
                                                                               zclAttrName,
                                                                               attrVal))
            elif respCode == zcl.statusCodes['ZCL_UNSUPPORTED_GENERAL_COMMAND']:
                print('{0},{1},{2:32},{3:20},ZCL_UNSUPPORTED_ATTRIBUTE'.format(attrId,
                                                                               attrType,
                                                                               zclAttrName,
                                                                               attrVal))
    ...
    if state and respCode == zcl.statusCodes['SUCCESS']:
        if len(bindings) > 0:
            for bind in bindings:
                if bind is not None: print(bind)
        else:
...
def resetSedAttrs(params):
    ...
    if params['nodeType'] == 'ZED':
        ...
        nodeEp = params['pollControlEp']
        respState, _, _ = setAttribute(nodeId, nodeEp, pollControlClustId, 'server', checkInId, checkInType,
                                       myIntervalHex)
        if not respState:
...
def resetFastPoll(longPoll, checkinInterval, nodeId, ep, context=None):
    """
    """
    ...
    print('\nUnbinding poll control cluster..')
    pollCluster = '0020'
    dstEp = '01'
    respState, respCode, respValue = getEUI(nodeId, nodeId)
    if respState and respCode == zcl.statusCodes['SUCCESS']:
    ...
    if respState and respCode == zcl.statusCodes['SUCCESS']:
        ...
        print("getEUI error. {}".format(respValue))
    respState, respCode, respValue = setUnBind(nodeId, nodeEUI, srcEp, pollCluster, controllerEUI, dstEp)

    # Reset check-in interval
    myIntervalHex = longPoll
    nodeEp = ep
    respState, _, _ = setChekinAttribute(nodeId, nodeEp, pollControlClustId, 'server', checkInId, checkInType,
                                         myIntervalHex)
    if not respState:
        print("Reset of short check-in interval failed")
        if context is not None:
            context.reporter.ReportEvent("Event Log", "Reset of long poll interval failed", "FAIL")
            context.reporter.ReportEvent("Event Log", "Terminating", "DONE")
        exit()

    # Reset long poll interval
    myIntervalHex = checkinInterval
    nodeEp = ep
    respState, _, _ = setLongPollInterval(nodeId, nodeEp, myIntervalHex)
    if not respState:
        print("Reset of long poll interval failed")
...
def setAttrReport(nodeId, nodeEp, params, myCluster, myAttribute, minRep, maxRep, changeRep):
    ...
    respState, respCode, respValue = setAttributeReporting(nodeId, nodeEp, myClusterId, attrId, minRep, maxRep,
                                                           changeRep)
    if not respState or respCode != zcl.statusCodes['SUCCESS']:
...
def setCompletFastPoll(nodeId, srcEp , apiFlag = False):
    # Setup binding
    # Wait for checkin - setup fast poll
    print('\nSetting poll control binding..')
    pollCluster = '0020'
    ...
    if not respState:
        print("Get check-in interval failed")
        exit()
    checkInInt = respValue

    # Set a short check-in interval to keep device in fast poll for duration of this getTelemetry
    myInterval = 30
    myIntervalHex = "{:08X}".format(myInterval * 4)
    respState, respCode, respValue = setChekinAttribute(nodeId, srcEp, pollControlClustId, 'server', checkInId,
                                                        checkInType,
                                                        myIntervalHex)
    if not respState:
        print("Setting short check-in interval failed")
    ...
    print("\n*** Long poll interval and check-in interval have been modified to allow attribute dump")
    print("Original long poll interval = {}".format(longPollInt))
    print("Original check-in interval  = {}\n".format(checkInInt))
    ...
    if not resp:
        ...
        if not apiFlag:
            ...
            return None,None
    # Wait for a checkIn so that device receives fast poll start
    fpStarted = waitForCheckIn(nodeId, srcEp)
    if not fpStarted:
        print("Fast Poll failed to start")
        if not apiFlag:
            exit()
...
if __name__ == "__main__":
    ...
    params = getInitialData(nodeId, fastPoll=fp, printStatus=True)
