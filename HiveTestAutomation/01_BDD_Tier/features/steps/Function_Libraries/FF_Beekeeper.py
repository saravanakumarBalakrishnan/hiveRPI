...
class apiCredentials(object):
    ...
    def __init__(self,
        ...
        self.beekeeperURL = self.servers[apiServer]['bkurl']
    ...
    def loadServers(self, apiServer):
        ...
        serverAttributes['bkurl'] = utils.getAttribute('common', 'BeekeeperURL', Env=strCurrentServer)
...
class accountClass(object):
            ...
            if len(hubs) == 0:
                ...
                for device in devicesStatus:
                    if device != 'hub':
                        ...
                        if myDev.battery is not None: myDev.battery = round(myDev.battery, 2)
    ...
    def printAccount(self):
        ...
        if self.upgrading:
            ...
            hubString = '{0:35},{1:19},{2},{3},{4}'.format(self.username, self.hubId, self.userId, hubStatus,
                                                           self.version)
...
def myApiCall(myApiCmd, myParams, myMethod, myNumOfRetries=1):
    ...
    if API_CREDENTIALS is None:
        ...
        exit()
    elif API_CREDENTIALS.apiCookie is None and myApiCmd != '/login':
...
def myApiCall_Old(myApiCmd, myParams, myMethod, myNumOfRetries=1):
    ...
    if myMethod == 'POST':
        if myParams is None:
            ...
            params = urllib.parse.urlencode(myParams).encode(
                'utf-8')  # params must be type byte (utf-8) for the api call
    ...
    if API_CREDENTIALS.apiCookie is not None:
...
def createCredentialsBeekeeper(myApiServer, myApiCaller=None, username=None, password=None):
    ...
    if API_CREDENTIALS.apiUsername is not None:
...
class superUserSessionObject(object):
    def __init__(self):
        ...
        # self.sessionId, self.userId, self.latestSupportedApiVersion, self.statusCode, self.response = getSessionToken(
        #    self.username, self.password)

        self.sessionId, self.userId, self.statusCode, self.response = getSessionToken(
            self.username, self.password)
        # self.headers['X-Omnia-Access-Token'] = self.sessionId
        self.headers['Authorization'] = self.sessionId
        return
...
class sessionObject(object):
    def __init__(self):
        ...
        self.sessionId, self.userId, self.statusCode, self.response, self.userName, self.firstName, self.lastName, \
        self.postcode, self.email, self.mobile, self.phone, \
        self.timezone, self.country, self.countryCode, self.locale, self.temperatureUnit, self.status, self.failuresEmail, self.failuresSMS, \
        self.warningsEmail, self.warningsSMS, self.nightAlerts = getSessionToken(
            self.username, self.password)
...
def getSessionToken(username, password):
    ...
    url = API_CREDENTIALS.apiUrl + '/cognito/login'
    # payload = json.dumps({'sessions': [{'username': username, 'password': password}]})
    payload = json.dumps({'username': username, 'password': password})
    headers = {'Accept': '*/*',
	           'User-Agent':'HiveRNApp/10.37.0',
               'Content-Type': 'application/json'}
    r = requests.post(url, data=payload, headers=headers)
    print(payload, headers , r.status_code)
    ...
    temperatureUnit = session['user']['temperatureUnit']
    status = session['status']
    failuresEmail = session['alerts']['failuresEmail']
    failuresSMS = session['alerts']['failuresSMS']
    warningsEmail = session['alerts']['warningsEmail']
    warningsSMS = session['alerts']['warningsSMS']
    nightAlerts = session['alerts']['nightAlerts']
    if 'latestSupportedApiVersion' in session:
        latestSupportedApiVersion = session['latestSupportedApiVersion']

    return sessionId, userId, r.status_code, r.text, userName, firstName, lastName, postcode, email, mobile, phone, \
           timezone, country, countryCode, locale, temperatureUnit, status, failuresEmail, failuresSMS, \
           warningsEmail, warningsSMS, nightAlerts  # , latestSupportedApiVersion


def deleteSessionV6(session):
    """ Logout from session
    """
    url = API_CREDENTIALS.apiUrl + '/cognito/login/{}'.format(session.sessionId)
...
def getProductsBEE(session):
    """
        """
    url = API_CREDENTIALS.beekeeperURL + '/products'
    r = requests.get(url, headers=session.headers)

    if r.status_code != 200:
        print("ERROR in getProducts(): ", r.status_code, r.reason, r.url, r.text)
        exit()
    return r.json()


def getDevicesBEE(session):
    """
        """
    url = API_CREDENTIALS.beekeeperURL + '/devices'
    r = requests.get(url, headers=session.headers)

    if r.status_code != 200:
        print("ERROR in getDevicesBEE(): ", r.status_code, r.reason, r.url, r.text)
        exit()
    return r.json()


def getContactsBEE(session):
    """
            """
    url = API_CREDENTIALS.beekeeperURL + '/contacts'
    r = requests.get(url, headers=session.headers)

    if r.status_code != 200:
        print("ERROR in getContactsBEE(): ", r.status_code, r.reason, r.url, r.text)
        exit()
    return r.json()


def getAttributesBEE(session, userID, uri):
    """
            """
    url = API_CREDENTIALS.beekeeperURL + '/user/' + userID + '/attributes/' + uri
    r = requests.get(url, headers=session.headers)

    if r.status_code != 200:
        print("ERROR in getAttributesBEE(): ", r.status_code, r.reason, r.url, r.text)
        exit()
    return r.json()


def updateContactsBEE(session, oUpdateReqList):
    """
    """
    payload = json.dumps({"contacts": oUpdateReqList})

    url = API_CREDENTIALS.beekeeperURL + '/contacts/'
    r = requests.put(url, headers=session.headers, data=payload)

    if r.status_code != 200:
        print("ERROR in updateContactsBEE(): ", r.status_code, r.reason, r.url, r.text)
        exit()
    return r.json()


def addContactsBEE(session, oUpdateReqList):
    """
    """
    payload = json.dumps({"contacts": oUpdateReqList})

    url = API_CREDENTIALS.beekeeperURL + '/contacts/'
    r = requests.post(url, headers=session.headers, data=payload)

    if r.status_code != 200:
        print("ERROR in addContactsBEE(): ", r.status_code, r.reason, r.url, r.text)
        exit()
    return r.json()


def deleteContactsBEE(session, oUpdateReqList):
    """
    """
    payload = json.dumps({"contacts": oUpdateReqList})

    url = API_CREDENTIALS.beekeeperURL + '/contacts/'
    r = requests.delete(url, headers=session.headers, data=payload)

    if r.status_code != 200:
        print("ERROR in deleteContactsBEE(): ", r.status_code, r.reason, r.url, r.text)
        exit()
    return r.json()


def getHolidayModeBEE(session):
    """
        """
    url = API_CREDENTIALS.beekeeperURL + '/holiday-mode'
    r = requests.get(url, headers=session.headers)

    if r.status_code != 200:
        print("ERROR in getHolidayMode(): ", r.status_code, r.reason, r.url, r.text)
        exit()
    return r.json()


def startHolidayModeBEE(session, oUpdateReq):
    """
    """
    payload = json.dumps(oUpdateReq)

    url = API_CREDENTIALS.beekeeperURL + '/holiday-mode'
    r = requests.post(url, headers=session.headers, data=payload)

    if r.status_code != 200:
        print("ERROR in startHolidayModeBEE(): ", r.status_code, r.reason, r.url, r.text)
        exit()
    return r.json()


def endHolidayModeBEE(session):
    """
    """
    # payload = json.dumps({"contacts" : oUpdateReqList})
...
def updateNodeBEE(session, oUpdateReqList, oUpdateNodeURL):
    """
    """
    payload = json.dumps(oUpdateReqList)

    url = API_CREDENTIALS.beekeeperURL + oUpdateNodeURL
    r = requests.post(url, headers=session.headers, data=payload)

    if r.status_code != 200:
        print("ERROR in updateNodeBEE(): ", r.status_code, r.reason, r.url, r.text)
        exit()
    return r.json()
...
def getUsersV6(session):
    """
    """
    url = API_CREDENTIALS.apiUrl + '/omnia/users/'
    r = requests.get(url, headers=session.headers)

    if r.status_code != 200:
        print("ERROR in getUsers(): ", r.status_code, r.reason, r.url, r.text)
        exit()
    return r.json()
...
def setModeV6(session, nodeId, setMode, targetHeatTemperature=None, scheduleLockDuration=60):
    ...
    if 'MANUAL' in setMode or 'ON' in setMode:
        strPUTJson = {"nodes": [{"attributes": {"activeHeatCoolMode": {"targetValue": "HEAT"},
                                                "activeScheduleLock": {"targetValue": "true"}}}]}
    elif 'AUTO' in setMode:
        strPUTJson = {"nodes": [{"attributes": {"activeHeatCoolMode": {"targetValue": "HEAT"},
                                                "activeScheduleLock": {"targetValue": "false"}}}]}
    elif 'OFF' in setMode:
        strPUTJson = {"nodes": [{"attributes": {"activeHeatCoolMode": {"targetValue": "OFF"},
                                                "activeScheduleLock": {"targetValue": "true"}}}]}
    elif 'BOOST' in setMode:
        if targetHeatTemperature is not None:
            strPUTJson = {"nodes": [{"attributes": {"activeHeatCoolMode": {"targetValue": "BOOST"},
                                                    "scheduleLockDuration": {"targetValue": scheduleLockDuration},
                                                    "targetHeatTemperature": {"targetValue": targetHeatTemperature}}}]}
        else:
            strPUTJson = {"nodes": [{"attributes": {"activeHeatCoolMode": {"targetValue": "BOOST"},
                                                    "scheduleLockDuration": {"targetValue": scheduleLockDuration}}}]}
    else:
...
def getUserSearch(mySearchString, myIndex, myQuantity=100):
    ...
    myParams = {'query': mySearchString, 'fieldSet': 'default hub user', 'quantity': str(myQuantity),
                'first': str(myIndex)}
...
def getColdAlert(myUsername, myHubId, myDevice):
    ...
    """
    myApiCmd = '/users/{0}/hubs/{1}/devices/all/{2}/events/TooCold/behaviors/ALL'.format(myUsername, myHubId,
                                                                                         myDevice.id)
    jsonResponse, state = myApiCall(myApiCmd, None, 'GET')
    return jsonResponse, state
...
def getHotAlert(myUsername, myHubId, myDevice):
    ...
    """
    myApiCmd = '/users/{0}/hubs/{1}/devices/all/{2}/events/TooHot/behaviors/ALL'.format(myUsername, myHubId,
                                                                                        myDevice.id)
    jsonResponse, state = myApiCall(myApiCmd, None, 'GET')
    return jsonResponse, state
...
def postHeatSchedule(myAccount, myDeviceId, mySched, myNumOfEvents):
    ...
    myParams = {'mode': 'HEAT', 'temperatureUnit': 'C', 'numberOfSetpoints': str(myNumOfEvents)}
...
def postWaterSchedule(myUsername, myDeviceId, mySched, myNumOfEvents):
    ...
    """
    # Build the parameter set
    myParams = {'temperature': 'C', 'numberOfEvents': str(myNumOfEvents)}
...
def setUserSubcription(session, BEEsession, strSubscription):
    """ Update the user Subcription
        PUT /1.0/internal/entitlement/{userid}
        Payload = { "LeakAlerts":true, "OngoingWarranty":true, "ProductDiscount":true, "UpgradeDiscount":true, "SMS":true, "HeatingFailureAlerts":true, "EarlyAccess":true, "FreeDelivery":true, "MimicMode":true, "subscriptions":[ { "name":"LeakAlertPlan", "status":"OK", "entitlements":[ "LeakAlerts" ] }, { "name":"HiveLive", "status":"OK", "entitlements":[ "OngoingWarranty", "ProductDiscount", "UpgradeDiscount", "SMS", "EarlyAccess", "FreeDelivery", "MimicMode" ] } ] }
        200 = Success

    """
    blnSuccess = True
    resp = getUsersV6(session)
    headers = BEEsession.headers
    strKey = utils.getAttribute('', 'BK_API_Key')
    if strKey == "Not Avaliable": strKey = ''
    headers.update({'x-api-key': strKey})
    if strSubscription == "HIVE LIVE":
        payload = json.dumps(
            {"ActionsDaylight": 'true', "ActionsMultiOutput": 'true', "OngoingWarranty": 'true',
             "ProductDiscount": 'true', "UpgradeDiscount": 'true', "SMS": 'true',
             "HeatingFailureAlerts": 'true', "EarlyAccess": 'true', "FreeDelivery": 'true', "MimicMode": 'true',
             "subscriptions": [{"name": "HiveLive", "status": "OK",
                                "entitlements": ["ActionsDaylight", "ActionsMultiOutput", "OngoingWarranty",
                                                 "ProductDiscount", "UpgradeDiscount", "SMS",
                                                 "EarlyAccess", "FreeDelivery", "MimicMode"]}]})
    elif strSubscription == "LEAK ALERT PLAN":
        payload = json.dumps({"LeakAlerts": 'true', "subscriptions": [
            {"name": "LeakAlertPlan", "status": "OK", "entitlements": ["LeakAlerts"]}]})
    else:
        print("No Entitlements provided")

    url = API_CREDENTIALS.beekeeperURL + '/internal/entitlement/{}'.format(resp['users'][0]['id'])
    r = requests.post(url, headers=BEEsession.headers, data=payload)
    if r.status_code != 200:

        print("ERROR in setUserSubcription(): ", r.status_code, r.reason, r.url, r.text)
        blnSuccess = False
        exit()
    else:
        blnSuccess = True
...
def removeUserSubcription(session, BEEsession, strSubscription):
    """ Remove the user Subcription
        PUT /1.0/internal/entitlement/{userid}
        Payload = { "LeakAlerts":true, "OngoingWarranty":true, "ProductDiscount":true, "UpgradeDiscount":true, "SMS":true, "HeatingFailureAlerts":true, "EarlyAccess":true, "FreeDelivery":true, "MimicMode":true, "subscriptions":[ { "name":"LeakAlertPlan", "status":"OK", "entitlements":[ "LeakAlerts" ] }, { "name":"HiveLive", "status":"OK", "entitlements":[ "OngoingWarranty", "ProductDiscount", "UpgradeDiscount", "SMS", "EarlyAccess", "FreeDelivery", "MimicMode" ] } ] }
        200 = Success
    ...
    if strSubscription == "HIVE LIVE":
        ...
        payload = json.dumps(
            {"OngoingWarranty": 'true', "ProductDiscount": 'true', "UpgradeDiscount": 'true', "SMS": 'true',
             "HeatingFailureAlerts": 'true', "EarlyAccess": 'true', "FreeDelivery": 'true', "MimicMode": 'true',
             "subscriptions": [{"name": "HiveLive", "status": "OK",
                                "entitlements": ["OngoingWarranty", "ProductDiscount", "UpgradeDiscount", "SMS",
                                                 "EarlyAccess", "FreeDelivery", "MimicMode"]}]})
    else:
        print("No Entitlements provided")
    ...
    if r.status_code != 200:
        ...
        print("ERROR in removeUserSubcription(): ", r.status_code, r.reason, r.url, r.text)
        blnSuccess = False
        exit()
    else:
        blnSuccess = True
...
def getActionTemplatesAndCount(session, BEEsession, Category, userDeviceCount):
    """ Get the action templates
        GET /1.0/action-templates

    """
    blnSuccess = True

    categories = {'Welcome Home': 'WelcomeHomeUK', 'Comfort': 'comfort', 'Reassurance': 'reassurance',
                  'Efficiency': 'efficiency', 'thermostat': 'heating', 'plug': 'plug',
                  'lights': 'light', 'motion-sensor': 'motion-sensor', 'contact-sensor': 'contact-sensor',
                  'hotwater': 'hotwater',
                  'Build Your Own': 'canvas'

                  }
    devices = {'motion-sensor', 'contact-sensor', 'plug', 'lights', 'thermostat', 'hotwater'}
    ...
    counterPaid = 0
    resp = getUsersV6(session)
    userLocale = format(resp['users'][0]['locale'])

    if userLocale == "en_US":
        categories['Welcome Home'] = 'WelcomeHomeUS'
        categories['Thermostat'] = ['humidity', 'nathermostat', 'temperature']
    if userLocale == "en_CA":
        categories['Welcome Home'] = 'WelcomeHomeUS'
        categories['Thermostat'] = ['humidity', 'nathermostat', 'temperature']
    elif userLocale == "en_GB":
        categories['Welcome Home'] = 'WelcomeHomeUK'
        categories['Thermostat'] = ['heating', 'temperature']
    elif userLocale == "it_IT":
        categories['Welcome Home'] = 'WelcomeHomeUK'
        categories['Thermostat'] = ['heating', 'temperature']
    elif userLocale == "ga_IE":
        categories['Welcome Home'] = 'WelcomeHomeUK'
        categories['Thermostat'] = ['heating', 'temperature']
    else:
        print('Welcome Home templates not available for' + userLocale + ' country')

    url = API_CREDENTIALS.beekeeperURL + '/action-templates'
    r = requests.get(url, headers=BEEsession.headers)
    oTemplates = r.json()
    print("Total templates are  : " + str(len(oTemplates['templates'])))
    if r.status_code != 200:
        print("ERROR in getActionTemplateCount(): ", r.status_code, r.reason, r.url, r.text)
        blnSuccess = False
        exit()
    else:
        if Category == "All actions":

            for oTemplate in oTemplates['templates']:
                oTemplateTags = oTemplate['meta']['tags']
                for i in range(0, len(oTemplateTags)):
                    for device in devices:
                        if categories[device] == oTemplate['meta']['tags'][i]:
                            if userDeviceCount[device] > 0:  # decides the available template
                                counterAvailable = counterAvailable + 1
                                availableTemplates.append(oTemplate)
                                continue
                            else:
                                counterUnavailable = counterUnavailable + 1
                                unavailableTemplates.append(oTemplate)  # decides the unavailable template
                                continue
                        else:
                            continue
            ...
            for oTemplate in oTemplates['templates']:
                oTemplateTags = oTemplate['meta']['tags']
                for i in range(0, len(oTemplateTags)):
                    for oThermtag in categories['Thermostat']:
                        if oThermtag == oTemplate['meta']['tags'][i]:
                            if str(len(oTemplate['entitlements'])) == '0':
                                counterFree = counterFree + 1
                                freeTemplates.append(oTemplate)
                            else:
                                counterPaid = counterPaid + 1
                                paidTemplates.append(oTemplate)
                        else:
                            continue
        else:
            for oTemplate in oTemplates['templates']:
                oTemplateTags = oTemplate['meta']['tags']
                for i in range(0, len(oTemplateTags)):
                    if categories[Category] == oTemplate['meta']['tags'][i]:
                        if str(len(oTemplate['entitlements'])) == '0':  # decides the free template
                            counterFree = counterFree + 1
                            freeTemplates.append(oTemplate)
                        else:
                            counterPaid = counterPaid + 1
                            paidTemplates.append(oTemplate)  # decides the paid template
                    else:
                        continue

        ''''#Separate the build you own tagged and non build you own tagged templates within free templates
        freeTemplates = json.dump(freeTemplates)
        for oTemplate in freeTemplates['templates']:
            oTemplateTags = oTemplate['meta']['tags']
            for i in range(0, len(oTemplateTags)):
                if 'canvas' == oTemplate['meta']['tags'][i]:
                    counterBYOFree = counterBYOFree + 1
                    BYOfreeTemplates.append(oTemplate)
                    continue
                else:
                    counterNonBYOFree = counterNonBYOFree + 1
                    NonBYOfreeTemplates.append(oTemplate)
                    continue
        freeTemplates = BYOfreeTemplates + NonBYOfreeTemplates
        counterFree = counterBYOFree + counterNonBYOFree'''

        if Category == "All actions":
            counter = counterAvailable + counterUnavailable
            categoryTemplates = availableTemplates + unavailableTemplates
        else:
            counter = counterFree + counterPaid
            categoryTemplates = freeTemplates + paidTemplates
        print('The number of templates for ' + '"' + Category + '"' + '  are  ' + str(counter))
        return blnSuccess, str(counter), categoryTemplates, str(userLocale)


def setTemplateOrder(strCategory, templates):
    print("Setting template order")


def countFreeOrPaidActionTemplates(session, BEEsession, templateprice):
    """ Get the action templates
        GET /1.0/action-templates
    ...
    freeTemplates = []
    paidTemplates = []
    buildYourOwnTemplates = []
    welcomeHomeTemplates = []
    resp = getUsersV6(session)
    userLocale = format(resp['users'][0]['locale'])
    ...
    if r.status_code != 200:
        print("ERROR in countFreeOrPaidActionTemplates(): ", r.status_code, r.reason, r.url, r.text)
        blnSuccess = False
        exit()
    else:
        print('User Country is ' + userLocale)
        for oTemplate in oTemplates['templates']:
            if oTemplate['id'] != 'null':
                counter = counter + 1
            else:
                print('Problem in couting total templates')

        print('The total number of templates are  ' + str(counter))
        for oTemplate in oTemplates['templates']:
            if templateprice == 'free' or templateprice == 'paid':
                if str(len(oTemplate['entitlements'])) == '0':
                    counterFree = counterFree + 1
                    freeTemplates.append(oTemplate)
                else:
                ...
                if templateprice == 'build your own':
                    if 'canvas' in oTemplate['meta']['tags']:
                        counterBuildYourOwn = counterBuildYourOwn + 1
                        buildYourOwnTemplates.append(oTemplate)
                elif templateprice == 'welcome home':
                    if 'GB' in userLocale:
                        if 'WelcomeHomeUK' in oTemplate['meta']['tags']:
                            counterWelcomeHome = counterWelcomeHome + 1
                            welcomeHomeTemplates.append(oTemplate)
                    elif 'US' in userLocale:
                        if 'WelcomeHomeUS' in oTemplate['meta']['tags']:
                            counterWelcomeHome = counterWelcomeHome + 1
                            welcomeHomeTemplates.append(oTemplate)
                    else:
                        print('User locale is not correct')

        if templateprice == 'free':
            i = 1
            print('The number of free templates for are : ' + str(counterFree))
            for oTemplate in freeTemplates:
                print('\n' + str(i) + '.' + 'Id is : ' + oTemplate['id'] + '\t' + '  Name is : ' +
                      oTemplate['meta'][userLocale.replace('_', '-')]['name'])
                i = i + 1
        elif templateprice == 'paid':
            i = 1
            print('The number of paid templates for are : ' + str(counterPaid))
            for oTemplate in paidTemplates:
                print('\n' + str(i) + '.' + 'Id is : ' + oTemplate['id'] + '\t' + '  Name is : ' +
                      oTemplate['meta'][userLocale.replace('_', '-')]['name'])
                i = i + 1
        elif templateprice == 'build your own':
            i = 1
            print('Build your own Templates are : ' + str(counterBuildYourOwn))
            for oTemplate in buildYourOwnTemplates:
                print('\n' + str(i) + '.' + 'Id is : ' + oTemplate['id'] + '\t' + '  Name is : ' +
                      oTemplate['meta'][userLocale.replace('_', '-')]['name'])
                i = i + 1
        elif templateprice == 'welcome home':
            i = 1
            print('welcome home Templates are : ' + str(counterWelcomeHome))
            for oTemplate in welcomeHomeTemplates:
                print('\n' + str(i) + '.' + 'Id is : ' + oTemplate['id'] + '\t' + '  Name is : ' +
                      oTemplate['meta'][userLocale.replace('_', '-')]['name'])
                i = i + 1
        else:
