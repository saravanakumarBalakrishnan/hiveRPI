...
def myApiCall(myApiCmd, myParams, myMethod, myNumOfRetries=1):
    ...
    Keyword arguments:
        ...
        myParams (dict)         -- POST command parameters or None.  Parameters must be encoded as byte type (utf-8)
...
def createCredentials(myApiServer, myApiCaller=None, username=None, password=None):
    ...
    if API_CREDENTIALS.apiUsername is not None:
        if API_CREDENTIALS.platformVersion == 'V5':
            ...
            if '6' in sessionV6.latestSupportedApiVersion:
...
class superUserSessionObject(object):
    def __init__(self):
        self.username = utils.getAttribute('common', strAttributeName='superusername')
        self.password = utils.getAttribute('common', strAttributeName='superuserpwd')
...
class sessionObject(object):
    def __init__(self):
        ...
        self.headers = {'Accept': 'application/vnd.alertme.zoo-6.0+json',
                        'X-AlertMe-Client': 'Hive Android App v1.2.7.06',
                        'Content-Type': 'application/json'}
...
class sessionObjectV6dot5(object):
    def __init__(self):
        self.username = API_CREDENTIALS.apiUsername
        self.password = API_CREDENTIALS.apiPassword
        self.headers = {'Accept': 'application/vnd.alertme.zoo-6.5+json',
                        'X-AlertMe-Client': 'KG',
                        'User-Agent': 'HiveRNApp/10.37.0',
                        'Content-Type': 'application/json'}
...
def getSessionToken(username, password):
    ...
    print('inside alertmelogin')
    url = 'https://beekeeper-intprod.hivehome.com/1.0/cognito/login'
    ...
    print("alert",payload, headers , r.status_code)
...
def deleteSessionV6(session):
    ...
    url = 'https://api.internalprod.zoo.alertme.com/omnia/auth/sessions/{}'.format(session.sessionId)
    r = requests.delete(url, headers=session.headers)
    print("session inside delte", session.headers)
    if r.status_code != 200:
...
def getNodesV6(session):
    ...
    """
    print("getting Nodes")
    url = 'https://api.internalprod.zoo.alertme.com/omnia/nodes'
    r = requests.get(url, headers=session.headers)
    print("hi",session.headers)
    if r.status_code != 200:
...
def getContactsV6(session):
    """
    """
    url = API_CREDENTIALS.apiUrl + '/omnia/contacts/'
    r = requests.get(url, headers=session.headers)
...
def setActiveLightState(session, nodeId, state):
    ...
    success = False
    # payload = json.dumps({"nodes": [{"attributes": {"state": {"targetValue": state}}}]})
    payload = json.dumps({"nodes": [{"features": {"on_off_device_v1":{"mode":{"targetValue": state}}}}]})
    url = 'https://api.internalprod.zoo.alertme.com:8443' + '/omnia/nodes/{}'.format(nodeId)
    r = requests.put(url, headers=session.headers, data=payload)
    if r.status_code == 200: success = True
    return r, success
...
#     payload = json.dumps({"nodes": [{"attributes": {"brightness": {"targetValue": intBrightness}}}]})
    payload = json.dumps({"nodes": [{"features":{"dimmable_light_v1": {"brightness": {"targetValue": intBrightness}}}}]})
#     url = API_CREDENTIALS.apiUrl + '/omnia/nodes/{}'.format(nodeId)
    url = 'https://api.internalprod.zoo.alertme.com:8443/omnia/nodes/{}'.format(nodeId)
    r = requests.put(url, headers=session.headers, data=payload)
    print("BritnessStatus",r)
    if r.status_code == 200: success = True
    return r, success
...
#     payload = json.dumps({"nodes": [{"attributes": {"colourTemperature": {"targetValue": intColorTemperature}}}]})
    payload = json.dumps({"nodes": [{"features":{"temperature_tunable_light_v1": {"colourTemperature": {"targetValue": intColorTemperature}}}}]})

#     url = API_CREDENTIALS.apiUrl + '/omnia/nodes/{}'.format(nodeId)
    url = 'https://api.internalprod.zoo.alertme.com:8443/omnia/nodes/{}'.format(nodeId)
    r = requests.put(url, headers=session.headers, data=payload)
    print("ColorTempResponse",r)
    if r.status_code == 200: success = True
...
def updateLeakStatus(strNativeIdentifier, strNotification):
    strKey = utils.getAttribute('', 'API_Key')
    if strKey == "Not Avaliable": strKey = ''
    API_CREDENTIALS.apiCookie = {'x-api-key': strKey,
                                 'Content-Type': 'application/json'}
    myApiCmd = API_CREDENTIALS.apiUrl.replace('https://api.', 'https://nodes.') + '/simulate/notification'
    myPayload = json.dumps({"node": strNativeIdentifier, "notification": strNotification})
    r = requests.post(myApiCmd, data=myPayload, headers=API_CREDENTIALS.apiCookie)
    if r.status_code != 200:
        print("ERROR in updateLeakStatus(): ", r.status_code, r.reason, r.url, r.text)
        print(r.text)


def updateLeakStatusFromLP(strNativeIdentifier, strNotification):
    headers = {'Content-Type': 'application/json'}
    myApiCmd = API_CREDENTIALS.apiUrl.replace('https://api.', 'https://mqtt.') + '/leak-notification'
    myApiCmd = myApiCmd.replace('api-prod', 'mqtt.prod')
    dtmTimeStamp = datetime.datetime.utcnow() - datetime.timedelta(seconds=3)
    myPayload = json.dumps({"uid": strNativeIdentifier, "notification": strNotification, "timestamp": str(dtmTimeStamp),
                            "durationSeconds": 2043})
    r = requests.post(myApiCmd, data=myPayload, headers=headers)
    if r.status_code != 200:
        print("ERROR in updateLeakStatusFromLP(): ", r.status_code, r.reason, r.url, r.text)
        print(r.text)
    return dtmTimeStamp


def getEvents(session, strDeviceType=None):
    toTime = datetime.datetime.now()

    if strDeviceType == 'MOTION SENSOR' or strDeviceType == 'CONTACT SENSOR':
        oEventLogsForWeekDict = {}
        oEventLogsForDayList = []
        toTimeToString = str(toTime)
        print(toTimeToString)
        hours1 = int(str(toTimeToString[11] + toTimeToString[12]))
        minutes1 = int(str(toTimeToString[14] + toTimeToString[15]))
        seconds1 = int(str(toTimeToString[17] + toTimeToString[18]))
        milliseconds1 = int(str(toTimeToString[20:]))
        for oDay in range(6, -1, -1):
            fromTime = datetime.datetime.now() - datetime.timedelta(days=oDay) - datetime.timedelta(
                hours=hours1) - datetime.timedelta(minutes=minutes1) - datetime.timedelta(
                seconds=seconds1) - datetime.timedelta(microseconds=milliseconds1)
            toTime = fromTime + datetime.timedelta(days=1)
            oEventNode = getEventsWithFilter(session, fromTime, toTime)
            oEventLogsForDayList = oEventNode
            oDayIndex = "Day %d" % (oDay + 1)
            oEventLogsForWeekDict.update({oDayIndex: oEventLogsForDayList})
            print(oEventLogsForWeekDict)
        return oEventLogsForWeekDict
    else:
        fromTime = datetime.datetime.now() - datetime.timedelta(minutes=3)
        oEventNode = getEventsWithFilter(session, fromTime, toTime)
        return oEventNode


def getEventsWithFilter(session, fromTime, toTime):
    unixFromTime = int(time.mktime(fromTime.timetuple()) * 1000)
    unixToTime = int(time.mktime(toTime.timetuple()) * 1000)
    myParams = {'fromTime': unixFromTime, 'toTime': unixToTime}
    url = API_CREDENTIALS.apiUrl + '/omnia/events/'
    r = requests.get(url, params=myParams, headers=session.headers)
    if r.status_code != 200:
        print("ERROR in getEvents(): ", r.status_code, r.reason, r.url, r.text)
        exit()
    return r.json()

def getEventsForDevice(session, DeviceNodeId, limit):
    url = API_CREDENTIALS.apiUrl + '/omnia/events?limit='+str(limit)+'&source='+DeviceNodeId
    r = requests.get(url, headers=session.headers)
    if r.status_code != 200:
        print("ERROR in getEvents(): ", r.status_code, r.reason, r.url, r.text)
        exit()
...
def putAlertSettings(session, dictPayload):
    """
    """
    blnSuccess = True
    resp = getRulesV6(session)
    for rNode in resp['rules']:
        nodeID = rNode['id']
        url = API_CREDENTIALS.apiUrl + '/omnia/rules/' + nodeID
        myPayload = json.dumps(dictPayload)
        r = requests.put(url, data=myPayload, headers=session.headers)

        if r.status_code != 200:
            print("ERROR in putAlertSettings(): ", r.status_code, r.reason, r.url, r.text)
            blnSuccess = False
            exit()
    return blnSuccess


def stopMimic(session):
    blnSuccess = True
    nodes = getNodesV6(session)

    for eachNode in nodes['nodes']:
        if 'fake.occupancy' in eachNode['nodeType']:
            nodeID = eachNode['id']
            url = API_CREDENTIALS.apiUrl + '/omnia/nodes/' + nodeID
            dictPayload = {"nodes": [{"attributes": {}, "features": {"enable_actions_v1":
                                                                         {"enabled": {"targetValue": False}}}}]}
            myPayload = json.dumps(dictPayload)
            r = requests.put(url, data=myPayload, headers=session.headers)

            if r.status_code != 200:
                print("ERROR in stopMimic(): ", r.status_code, r.reason, r.url, r.text)
                blnSuccess = False
            break
    return blnSuccess


def delMimic(session):
    blnSuccess = True
    nodes = getNodesV6(session)
...
def getTimeZone(session):
    """
    """
    url = API_CREDENTIALS.apiUrl + '/omnia/users?fields=timeZone'
    r = requests.get(url, headers=session.headers)
...
def updateUserDetails(session, Locale):
    """ Update the user details
        PUT /users/{id}
        Payload = { "users": [ { "username": "example", "email": "account@example.com", "firstName": "Example", "lastName": "User", "postcode": "SW1A 0AA", "locale": "en_GB", "timeZone": "Europe/London", "partnerId": "03d75be3-a133-4687-8f8d-bb248c11d2c5", "permissions": [ "ROLE_USER" ], "extCustomerLevel": 1, "alertSettings": { "failuresEmail": false, "failuresSMS": true, "warningsEmail": true, "warningsSMS": false, "nightAlerts": true } } ] }
        200 = Success

    """
    blnSuccess = True
    resp = getUsersV6(session)
    payload = json.dumps({"users": [Locale]})

    if 'users' in resp:
        url = API_CREDENTIALS.apiUrl + '/omnia/users/{}'.format(resp['users'][0]['id'])
        r = requests.put(url, headers=session.headers, data=payload)
        if r.status_code != 200:
            print("ERROR in updateUserDetails(): ", r.status_code, r.reason, r.url, r.text)
            blnSuccess = False
            exit()
    else:
        blnSuccess = False
