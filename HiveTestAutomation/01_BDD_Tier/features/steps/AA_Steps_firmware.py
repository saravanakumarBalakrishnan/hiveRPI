...
@when(
    u'the {DeviceName} of {DeviceType} is upgraded and downgraded between {DeviceVersion1:.2f} and {DeviceVersion2:.2f} infinitely and validated')
def upgrade_downgrade_firmware(context, DeviceName, DeviceType, DeviceVersion1, DeviceVersion2):
    ...
    if not os.path.exists(context.networkBasePath):
        ...
        if strFWFilePath1 != "" and strFWFilePath2 != "":
            ...
            while True:
                context.rFM.upgrade_or_owngrade_firmware(context.reporter, "Downgrade", strFWFilePath1, nodeId, ep,
                                                         DeviceType, DeviceVersion1)
                time.sleep(900)
                context.rFM.validate_firmware_version(context.reporter, "Downgrade", nodeId, ep, DeviceType,
                                                      DeviceVersion1)

                time.sleep(30)
                context.rFM.upgrade_or_owngrade_firmware(context.reporter, "Upgrade", strFWFilePath2, nodeId, ep,
                                                         DeviceType, DeviceVersion2)
                time.sleep(900)
                context.rFM.validate_firmware_version(context.reporter, "Upgrade", nodeId, ep, DeviceType,
                                                      DeviceVersion2)

        else:
            print(
                "Firmware file for given combination is not present in the share path. Please make sure file exist and rerun the test")
            return
...
def verifyMCU(context, nodeId, ep, DeviceVersion):
    ...
    if boolAppVersionFlag and boolFileVersionFlag:
        context.reporter.ReportEvent("Test Validation",
                                     "The Application version updated in the zigbee as " + strHexAppString, "Pass")
        context.reporter.ReportEvent("Test Validation",
                                     "The Current File version updated in the zigbee as " + strFileVersion, "Pass")
    else:
        context.reporter.ReportEvent("Test Validation",
                                     "The Application version updated in the zigbee as " + strActAppString, "Fail")
        context.reporter.ReportEvent("Test Validation",
                                     "The Current File version updated in the zigbee as " + strActFileString, "Fail")


@when(
    u'the below list of DeviceName of DeviceType is upgraded and downgraded between DeviceVersion1 and DeviceVersion2 infinitely with reboot and validated via Telegisis {PlugMacID}')
def upgrade_downgrade_firmware_forall(context, PlugMacID):
    intCntr = 0
    while True:
        intCntr = intCntr + 1
        ...
        for oRow in context.table:
            ...
            macId = dutils.getDeviceMACWithModel(DeviceType, True)
            myNodeId = dutils.getDeviceNodeWithMAC(DeviceType, macId)
            print(oRow['Reboot'])
            if not oRow['Reboot']:
                print('Not Reboot Test Case')
                reboot = False
            elif oRow['Reboot'] == 'True':
                print('Reboot Test Case with Plug')
                reboot = None
                plugMacId = PlugMacID
            else:
                plugMacId = ''
                reboot = True
                print('Reboot Test Case')
            ...
            if not os.path.exists(context.networkBasePath):
                ...
                if str(DeviceType).upper() == "GENERIC":
                    ...
                    macId = dutils.getDeviceMACWithModel(DeviceType, True)
                    nodeId = dutils.getDeviceNodeWithMAC(DeviceType, macId)
                    ep = get_ep_for_deviceType(DeviceType.upper())
                ...
                if not 'NA' in DeviceVersion1.upper():
                    ...
                    strFWFilePath1 = get_firmware_file(firmwareRootFilePath, DeviceType, DeviceVersion1)
                    context.rFM.upgrade_or_downgrade_firmware(context.reporter, "Downgrade", strFWFilePath1, nodeId, ep,
                                                              DeviceType, DeviceVersion1, reboot, plugMacId)
                ...
                if not 'NA' in DeviceVersion2.upper():
                    strFWFilePath2 = get_firmware_file(firmwareRootFilePath, DeviceType, DeviceVersion2)
                    context.rFM.upgrade_or_downgrade_firmware(context.reporter, "Upgrade", strFWFilePath2, nodeId, ep,
                                                              DeviceType, DeviceVersion2, reboot, plugMacId)
                    if DeviceType.upper() == "SLT4":
                        verifyMCU(context, nodeId, ep, DeviceVersion2)


@when(
    u'the below list of DeviceName of DeviceType is upgraded and downgraded between DeviceVersion1 and DeviceVersion2  with reboot and validated via Telegisis {PlugMacID}')
def upgrade_downgrade_firmware_forall(context, PlugMacID):
    intCntr = 0
    while True:
        intCntr = intCntr + 1
...
@when(
    u'the below list of DeviceName of DeviceType is upgraded and downgraded between DeviceVersion1 and DeviceVersion2 infinitely and validated via Telegisis')
def upgrade_downgrade_firmware_forall(context):
    intCntr = 0
    thread = threading.Thread(target=utils.validate_alarm_message,args=(context.reporter,))
        ...
        for oRow in context.table:
            ...
            if not os.path.exists(context.networkBasePath):
                ...
                if not 'NA' in DeviceVersion1.upper():
                    ...
                    strFWFilePath1 = get_firmware_file(firmwareRootFilePath, DeviceType, DeviceVersion1)
                    context.rFM.upgrade_or_downgrade_firmware(context.reporter, "Downgrade", strFWFilePath1, nodeId, ep,
                                                              DeviceType, DeviceVersion1, False, '')
                ...
                if not 'NA' in DeviceVersion2.upper():
                    strFWFilePath2 = get_firmware_file(firmwareRootFilePath, DeviceType, DeviceVersion2)
                    context.rFM.upgrade_or_downgrade_firmware(context.reporter, "Upgrade", strFWFilePath2, nodeId, ep,
                                                              DeviceType, DeviceVersion2, False, '')
                    if DeviceType.upper() == "SLT4":
                        verifyMCU(context, nodeId, ep, DeviceVersion2)
                    '''if 'SLT3' in DeviceType.upper(): time.sleep(0)#time.sleep(1500)
                    else: time.sleep(500)'''
...
@when(
    u'the below list of DeviceName of DeviceType is upgraded and downgraded between DeviceVersion1 and DeviceVersion2 via Telegisis')
def upgrade_downgrade_firmware_for_once(context):
    intCntr = 0
    while True:
        intCntr = intCntr + 1
        context.reporter.HTML_TC_BusFlowKeyword_Initialize('Upgrade-Downgrade Counter : ' + str(intCntr))
...
@then(
    u'the below list of DeviceName of DeviceType is upgraded and downgraded between DeviceVersion1 and DeviceVersion2 via Telegisis')
def upgrade_downgrade_firmware_forall_once(context):
    intCntr = 0
    while True:
        intCntr = intCntr + 1
        context.reporter.HTML_TC_BusFlowKeyword_Initialize('Upgrade-Downgrade Counter : ' + str(intCntr))
...
@when(
    u'the below list of DeviceName of DeviceType is upgraded and downgraded between DeviceVersion1 and DeviceVersion2 infinitely and validated via HUB')
        ...
        for oRow in context.table:
            ...
            if 'SLR' in DeviceType.upper():
                ...
                if 'SLR2' in DeviceType.upper():
                    context.waterEP.setModeViaAPI(nodeIdList[DeviceType], strModeToSet, targetHeatTemperature=None,
            ...
            # Downgrade device
            if not 'NA' in DeviceVersion1.upper():
                context.rFM.upgrade_or_downgrade_firmwareViaHUB(context.oThermostatEP, context.reporter, "Downgrade",
                                                                DeviceType, DeviceVersion1, False)
                if 'SLT3' in DeviceType.upper() or 'SLT3B' in DeviceType.upper():
                    time.sleep(600)
                    # power_cycle_hub()
                    print(strModeToSet)
                else:
                    if 'CL01' in DeviceType.upper():
                        if context.rFM.verifyfirmwareVersionsViaHUB(context.oThermostatEP, context.reporter,
                                                                    "Downgrade", DeviceType, DeviceVersion1):
                            # time.sleep(12000)
                            print()
                    else:
                        if context.rFM.verifyfirmwareVersionsViaHUB(context.oThermostatEP, context.reporter,
                            ...
                            # time.sleep(2500)
                            print(strModeToSet)
                context.rFM.validate_firmware_versionViaHUB(context.oThermostatEP, context.reporter, "Downgrade",
                                                            DeviceType, DeviceVersion1)

                if 'SLR' in DeviceType.upper():
                    if 'AUTO' in strModeToSet: targetHeatTemperature = \
                        oSchdUt.getCurrentTempFromSchedule(context.heatEP.getSchedule())[0]
                    heatWeeklyScheduleAfter = context.heatEP.getSchedule()
                    context.rFM.validateAndUpdateLog(context.reporter, context.heatEP, 'Test', strModeToSet,
                                                     targetHeatTemperature)

                    oSchdUt.validateSchedulesOfOtherWeekdays(context.reporter, oSchedule, heatWeeklyScheduleBefore,
                                                             heatWeeklyScheduleAfter)
                    if 'SLR2' in DeviceType.upper():
                        if 'AUTO' in strModeToSet: targetHeatTemperature = \
                            oSchdUt.getCurrentTempFromSchedule(context.waterEP.getSchedule())[0]
                        context.rFM.validateAndUpdateLog(context.reporter, context.waterEP, 'Test', strModeToSet,
                                                         targetHeatTemperature)
                        ...
                        oSchdUt.validateSchedulesOfOtherWeekdays(context.reporter, oSchedule, waterWeeklyScheduleBefore,
                                                                 waterWeeklyScheduleAfter)
                time.sleep(30)

            # Upgrade Device
            if not 'NA' in DeviceVersion2.upper():
                context.rFM.upgrade_or_downgrade_firmwareViaHUB(context.oThermostatEP, context.reporter, "Upgrade",
                                                                DeviceType, DeviceVersion2, False)
                if 'SLT3' in DeviceType.upper() or 'SLT3B' in DeviceType.upper():
                    time.sleep(600)
                    # power_cycle_hub()
                    ...
                    if 'CL01' in DeviceType.upper():
                        if context.rFM.verifyfirmwareVersionsViaHUB(context.oThermostatEP, context.reporter, "Upgrade",
                                                                    DeviceType, DeviceVersion1):
                            # time.sleep(12000)
                            print()
                    else:
                        if context.rFM.verifyfirmwareVersionsViaHUB(context.oThermostatEP, context.reporter, "Upgrade",
                            ...
                            # time.sleep(2500)
                            print()
                context.rFM.validate_firmware_versionViaHUB(context.oThermostatEP, context.reporter, "Upgrade",
                                                            DeviceType, DeviceVersion2)
                if 'SLR' in DeviceType.upper():
                    if 'AUTO' in strModeToSet: targetHeatTemperature = \
                        oSchdUt.getCurrentTempFromSchedule(context.heatEP.getSchedule())[0]
...
@when(
    u'the below list of DeviceName of DeviceType is upgraded and downgraded between DeviceVersion1 and DeviceVersion2 and validated via HUB')
def upgrade_downgrade_firmware_once_forallViaHUB(context):
    print("ViaHub\n")
    ...
    resp = ALAPI.getNodesV6(session)
    # nodeIdList = context.oThermostatEP.getNodeID(resp)
    nodeIdList = dutils.getNodeID(resp)
    ALAPI.deleteSessionV6(session)
    ...
    for oRow in context.table:
        ...
        # Downgrade device
        if not 'NA' in DeviceVersion1.upper():
            context.rFM.upgrade_or_downgrade_firmwareViaHUB(context, context.reporter,
                                                            "Downgrade",
                                                            DeviceType, DeviceVersion1, False)
            if 'SLT3' in DeviceType.upper() or 'SLT3B' in DeviceType.upper():
                time.sleep(600)
                ...
                if 'CL01' in DeviceType.upper():
                    if context.rFM.verifyfirmwareVersionsViaHUB(context, context.reporter,
                    ...
                    if context.rFM.verifyfirmwareVersionsViaHUB(context, context.reporter,
                        ...
                        # time.sleep(2500)
                        print(strModeToSet)
            context.rFM.validate_firmware_versionViaHUB(context, context.reporter, "Downgrade",
                                                        DeviceType, DeviceVersion1)
            ...
            if 'SLR' in DeviceType.upper():
                ...
                if 'SLR2' in DeviceType.upper():
                    ...
                    waterWeeklyScheduleBefore = oSchdUt.converWaterStateForSchedule(waterWeeklyScheduleBefore)
                    waterWeeklyScheduleAfter = oSchdUt.converWaterStateForSchedule(waterWeeklyScheduleAfter)
                    oSchdUt.validateSchedulesOfOtherWeekdays(context.reporter, oSchedule,
        ...
        # Upgrade Device
        if not 'NA' in DeviceVersion2.upper():
            context.rFM.upgrade_or_downgrade_firmwareViaHUB(context, context.reporter, "Upgrade",
                                                            DeviceType, DeviceVersion2, False)
            if 'SLT3' in DeviceType.upper() or 'SLT3B' in DeviceType.upper():
                # time.sleep(6000)
                # power_cycle_hub()
                ...
                if 'CL01' in DeviceType.upper():
                    ...
                    if context.rFM.verifyfirmwareVersionsViaHUB(context, context.reporter,
                        ...
                        # time.sleep(2500)
                        print()
            context.rFM.validate_firmware_versionViaHUB(context, context.reporter, "Upgrade",
                                                        DeviceType, DeviceVersion2)
            if 'SLR' in DeviceType.upper():
                if 'AUTO' in strModeToSet: targetHeatTemperature = \
                    oSchdUt.getCurrentTempFromSchedule(context.heatEP.getSchedule())[0]
...
@when(
    u'the below list of DeviceName of DeviceType is upgraded and downgraded between DeviceVersion1 and DeviceVersion2 with {strDeviceType} reboot using {strPlugMac} and validated via HUB')
def upgrade_downgrade_firmware_reboot_forallViaHUB(context, strDeviceType, strPlugMac):
    print("ViaHub\n")
    ...
    for oRow in context.table:
        ...
        if not os.path.exists(context.networkBasePath):
            print("Network drive is not mounted")
        if str(strDeviceType).upper() == "DEVICE":
            strDeviceType = "CL01"
        elif str(strDeviceType).upper() == "ROUTER":
            strDeviceType = "NANO2"
        # Downgrade device
        if not 'NA' in DeviceVersion1.upper():
            context.rFM.upgrade_or_downgrade_firmwareViaHUB(context.oThermostatEP, context.reporter,
                                                            "Downgrade",
                                                            DeviceType, DeviceVersion1, strPlugMac, strDeviceType)
            if 'SLT3' in DeviceType.upper() or 'SLT3B' in DeviceType.upper():
                time.sleep(600)
                # power_cycle_hub()
                print(strModeToSet)
        ...
        if not 'NA' in DeviceVersion2.upper():
            context.rFM.upgrade_or_downgrade_firmwareViaHUB(context.oThermostatEP, context.reporter, "Upgrade",
                                                            DeviceType, DeviceVersion2, strPlugMac, strDeviceType)
            if 'SLT3' in DeviceType.upper() or 'SLT3B' in DeviceType.upper():
                # time.sleep(6000)
                # power_cycle_hub()
                print()
...
def get_firmware_file(firmwareRootFilePath, DeviceType, DeviceVersion):
    ...
    if "SLR1" == DeviceType.upper():
        DeviceVersion = "V" + DeviceVersion.replace(".", "_")
    if "SLR2" == DeviceType.upper():
        DeviceVersion = "V" + DeviceVersion.replace(".", "_")
    if "SLR1B" in DeviceType.upper():
        DeviceVersion = DeviceVersion.replace(".", "")
    if "SLR2B" in DeviceType.upper():
        DeviceVersion = DeviceVersion.replace(".", "")
    if "SLT3" in DeviceType.upper():

        if "SLT3B" in DeviceType.upper():
    ...
    if "SLP2" in DeviceType.upper():
        if "SLP2C" in DeviceType.upper():
            DeviceVersion = DeviceVersion.replace(".", "")
        elif "SLP2B" in DeviceType.upper():
            DeviceVersion = DeviceVersion.replace(".", "")
        elif "SLP2D" in DeviceType.upper():
            DeviceVersion = DeviceVersion.replace(".", "")
        else:
            DeviceVersion = "SLP2_0" + DeviceVersion.replace(".", "")
    if "SLB2" in DeviceType.upper():
        DeviceVersion = DeviceVersion.replace(".", "")
