...
def setBothNotification(context,strSetTemp1,strSetTemp2,strClientType):
    ...
    if (5 <= int(strSetTemp1) <= 32) or (5 <= int(strSetTemp2) <= 32):
        print('High  & Low Temperature Alert')
        context.oThermostatEP.setHighNotification(strSetTemp1)
        context.oThermostatEP.setLowNotification(strSetTemp2)
    else :
        return False
