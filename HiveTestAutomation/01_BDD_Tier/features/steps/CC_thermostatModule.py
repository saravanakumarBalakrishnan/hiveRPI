...
COMMAND_TIMEOUT = datetime.timedelta(
    seconds=120)  # Max time to wait (in seconds) for states to resolve after sending commands
...
class thermostatEndpoint(object):
    ...
    def __init__(self, thermostat, nodeId, epType, epId):
        ...
        self.systemMode = self._initialiseAttribute('Thermostat Cluster', 'systemMode', )
        self._temperatureSetpointHold = self._initialiseAttribute('Thermostat Cluster', 'temperatureSetpointHold')
        self._temperatureSetpointHoldDuration = self._initialiseAttribute('Thermostat Cluster',
        ...
        if self.type == 'HEAT':
            ...
            self.frostProtectionSetpoint = self._initialiseAttribute('BG Cluster', 'frostProtectionSetpoint', True)
            # Previous modes are encoded 00=OFF, 01=MANUAL, 10=AUTO.  Use lastMode to get string mode
            self._previousHeatMode = self._initialiseAttribute('BG Cluster', 'previousHeatMode', True)
            self._previousWaterMode = self._initialiseAttribute('BG Cluster', 'previousWaterMode', True)
            self.previousHeatSetpoint = self._initialiseAttribute('BG Cluster', 'previousHeatSetpoint', True)

            # Holiday mode attributes
            self.holidayModeEnabled = self._initialiseAttribute('BG Cluster', 'holidayModeEnabled', True)
            self.holidayModeActive = self._initialiseAttribute('BG Cluster', 'holidayModeActive', True)
            self.holidaySetpoint = self._initialiseAttribute('BG Cluster', 'holidaySetpoint', True)
            # These ones are private because we build mode useful datetimes from them
            self._holidayStartDate = self._initialiseAttribute('BG Cluster', 'holidayStartDate', True)
            self._holidayStartTime = self._initialiseAttribute('BG Cluster', 'holidayStartTime', True)
            self._holidayEndDate = self._initialiseAttribute('BG Cluster', 'holidayEndDate', True)
            self._holidayEndTime = self._initialiseAttribute('BG Cluster', 'holidayEndTime', True)
    ...
    def setMode(self, myMode, mySetpoint=None, myDuration=1):
        ...
        if self.type == 'WATER' and mySetpoint is not None:
            self._setStatusCode("ERROR: Setpoint is not allowed for WATER.")
            return self.statusOk, self.statusCode

            # Only transitions to BOOST can include a setpoint
        if myMode != 'BOOST' and mySetpoint is not None:
            self._setStatusCode(
                "ERROR: setpoint is not allowed with mode switch from {} to {}".format(self.mode, myMode))
            return self.statusOk, self.statusCode
        ...
        if myMode == 'BOOST' and mySetpoint is None:
        ...
        # Set the wanted mode
        respState, _, respValue = AT.setMode(self.nodeId, self.epId, myMode, self.type, mySetpointFloat=mySetpoint,
        ...
        # Switch to MANUAL
            ...
            if mySetpoint is None: mySetpoint = self.MANUAL_DEFAULT
            self.model.occupiedHeatingSetpoint = tt.temperatureFloatToHexString(mySetpoint)
        ...
        # Switch to BOOST sets setpoint and an expire time
            ...
            if mySetpoint is None: mySetpoint = self.BOOST_DEFAULT
    ...
    def setSetpoint(self, mySetpoint):
        ...
        if not mySetpoint in self.allowedSetpoints:
            self._setStatusCode(
                "ERROR: Setpoint must be 1'C or in range {}-{}'C. sp={}".format(self.minSetpoint, self.maxSetpoint,
                                                                                mySetpoint))
            return self.statusOk, self.statusCode
    ...
    def setSchedule(self, schedule, boolStandaloneMode=False):
        ...
        for d in schedule:
            ...
            respState, _, respVal = AT.setWeeklySchedule(self.nodeId, self.epId, '0201', days[d], payload,
                                                         boolStandaloneMode)
            if not respState:
                self._setStatusCode('ERROR: setWeeklySchedule() has failed. {}'.format(respVal))
                return self.statusOk, self.statusCode
        self._scheduleLastUpdate = 0
        clust = '0201'
        for day in days:
            respState, _, respValue = AT.getWeeklySchedule(self.nodeId, self.epId, clust, days[day])
            if not respState:
    ...
    def setHoliday(self, startDatetime, endDatetime, setpoint):
        ...
        self.model.holidayModeStart = self._buildHolidayDatetimeUTC(params['holidayStartDate'],
                                                                    params['holidayStartTime'])
    ...
    def _checkSettingsPropagated(self):
        ...
        while not self.statusOk:
            # _If we have timeout then exit.
            nowTime = getTestTime(timeOffset)
            if nowTime > startTime + COMMAND_TIMEOUT:
                inGuardZone, remainingSeconds, eventName = self.model._guardZone(nowTime)
                myError = 'ERROR: Timeout. Settings have not propagated correctly. guardzone={}, eventName={}, remainingSeconds={}'.format(
                    inGuardZone,
                    eventName,
                    remainingSeconds)
    ...
    def _updateThermostatEp(self):
        ...
        self._temperatureSetpointHold = self._getAttributeFromRedis(self.epId, 'Thermostat Cluster',
                                                                    'temperatureSetpointHold')
        self._temperatureSetpointHoldDuration = self._getAttributeFromRedis(self.epId, 'Thermostat Cluster',
                                                                            'temperatureSetpointHoldDuration')
        self.thermostatRunningState = self._getAttributeFromRedis(self.epId, 'Thermostat Cluster',
        ...
        if self.type == 'HEAT':
            ...
            self.occupiedHeatingSetpoint = self._getAttributeFromRedis(self.epId, 'Thermostat Cluster',
        ...
        ep._initialiseAttribute('BG Cluster', 'previousHeatMode', True)
        ep._initialiseAttribute('BG Cluster', 'previousWaterMode', True)
        ep._initialiseAttribute('BG Cluster', 'previousHeatSetpoint', True)
        ...
        ep._initialiseAttribute('BG Cluster', 'holidayModeEnabled', True)
        ep._initialiseAttribute('BG Cluster', 'holidayModeActive', True)
        ep._initialiseAttribute('BG Cluster', 'holidaySetpoint', True)
        ep._initialiseAttribute('BG Cluster', 'holidayStartDate', True)
        ep._initialiseAttribute('BG Cluster', 'holidayStartTime', True)
        ep._initialiseAttribute('BG Cluster', 'holidayEndDate', True)
        ep._initialiseAttribute('BG Cluster', 'holidayEndTime', True)
    ...
    def _initialiseAttribute(self, myClustName, myAttrName, mspAttr=False):
        ...
        attrId = str(attrId).replace("_msp", "")
        print('debug: {}'.format(myAttrName))
        respState, _, respVal = AT.getAttribute(self.nodeId, self.epId, clustId, attrId, 'server', mspAttr)
        if not respState:
    ...
    def _getAttributeFromRedis(self, myEpId, myClusterName, myAttributeName):
        ...
        redisKey = "{0},{1},{2},{3}".format(self.nodeId, myEpId, clustId, str(attrId).replace("_msp", ""))
...
class thermostatEndpointModel(object):
    ...
    def checkModel(self):
        ...
        if ((self.mode == 'BOOST' or self.mode == 'OVERRIDE') and self.modeExpire is not None) or \
                (self.mode == 'HOLIDAY' and nowTime >= holidayModeEnd):
            if nowTime >= self.modeExpire:
                # Deal with the mode
        ...
        if holidayModeEnabled == '01':
            ...
            if holidayModeStart is None:
                ...
                # _This is the only mode that has a valid historical setpoint.
        ...
        if self.parentEp.type == 'HEAT':
            if self.frostProtectionSetpoint != self.parentEp.frostProtectionSetpoint:
                self._setStatusCode(
                    'Frost protection setpoint inconsistent. Expected={}'.format(self.frostProtectionSetpoint))
        ...
        if self.parentEp.type == 'HEAT':
            if self.mode == 'OFF':
                ...
                if self.occupiedHeatingSetpoint != self.parentEp.occupiedHeatingSetpoint:
                    self._setStatusCode("Setpoint inconsistent. Expected setpoint = {}'C".format(
                        tt.temperatureHexStringToFloat(self.occupiedHeatingSetpoint)))
        ...
        if self.parentEp.type == 'HEAT':
            ...
            if self.thermostatRunningState != self.parentEp.thermostatRunningState:
                self._setStatusCode('Heat relay states are inconsistent. Tstat={}, model={}'.format(
                    self.parentEp.thermostatRunningState,
                    self.thermostatRunningState))
        ...
        if self.mode == 'BOOST' or self.mode == 'HOLIDAY':
            ...
            if self.parentEp.type == 'HEAT':
                if self.lastSetpoint != self.parentEp.previousHeatSetpoint:
                    self._setStatusCode('Previous setpoint is inconsistent. Expected={}'.format(
                        tt.temperatureHexStringToFloat(self.lastSetpoint)))
...
def serialInit():
    ...
    AT.getInitialData(config.NODE_ID, fastPoll=True, printStatus=True)
...
def heatModeChangeTest(tstat, modeList, startMode, startSetpoint=None):
    for mode in modeList:
        ...
        if startSetpoint is not None:
            tstat.heatEP.setSetpoint(startSetpoint)
        print('Start Mode = {}, startSetpoint={}'.format(tstat.heatEP.mode,
                                                         tt.temperatureHexStringToFloat(
                                                             tstat.heatEP.occupiedHeatingSetpoint)))
...
def holidayTransitionsTest(tstat, simulateTime=False):
    ...
    for hm in heatModes:
        for wm in waterModes:
            ...
            holidayStart = (datetime.datetime.now() + datetime.timedelta(seconds=holidayStartOffset)).replace(second=0,
                                                                                                              microsecond=0)
            ...
            if tstat.heatEP.statusOk:
                tstat.update()
                tsPrint('Holiday Set. HEAT Mode={}, WATER Mode={}, Holiday Start={}, Holiday End={}'.format(
                    tstat.heatEP.mode, tstat.waterEP.mode, holidayStart, holidayEnd), 1)
            else:
            ...
            while holiday and datetime.datetime.now() <= timeout:
                ...
                if tstat.heatEP.mode != 'HOLIDAY' or tstat.waterEP.mode != 'HOLIDAY':
                    tsPrint('ERROR: Holiday has ended early. HEAT={}, WATER={}'.format(tstat.heatEP.mode,
                                                                                       tstat.waterEP.mode), 1)
                    exit()
                time.sleep(10)
