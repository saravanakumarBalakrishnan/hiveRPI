...
def getCurrentDevicestatus(context, nameContactSensor):
    ...
    context.currentStatus = context.oThermostatEP.currentCSStatus(nameContactSensor)
...
def getCurrentResponseAPI(context, nameContactSensor, DeviceType):
    context.reporter.HTML_TC_BusFlowKeyword_Initialize(
        'Validating API status for given ' + nameContactSensor + ' status')
    ...
    oDeviceNodeID = pUtils.getDeviceNodeByName(nameContactSensor)
    print(oDeviceNodeID, "\n")

    # Get the the final Status of Contact sensor from platform
    finalCSState = pUtils.getCSAttributes(oDeviceNodeID)

    if context.currentStatus.upper() == finalCSState.upper():
        strLog = "State on Client$$State on platform@@@" + context.currentStatus.upper() + "$$" + finalCSState.upper()
        strStatus = "PASS"
    else:
        strLog = "State on Client$$State on platform@@@||" + context.currentStatus.upper() + "$$" + finalCSState.upper()
        strStatus = "FAIL"
...
@when(u'User check {selectWeekDay} day back in the event logs in the Client')
def verifyGivenDayLog(context, selectWeekDay):
    context.reporter.HTML_TC_BusFlowKeyword_Initialize(
        'Hive user able to navigate to ' + selectWeekDay + ' day back in log section')
...
def verifyEventLog(context):
    context.reporter.HTML_TC_BusFlowKeyword_Initialize(
        'Hive user able to see the event logs state on log detail screen')
    ...
    while doLoop:
        ...
        if not AT.rxQueue.empty():
            ...
            if "CHECKIN" in resp:
                ...
                context.reporter.ReportEvent('Test Validation', Device + 'Checkin Counter: ' + str(strTCDuration),
                                             "DONE")
                intTCStartTime = time.monotonic()
