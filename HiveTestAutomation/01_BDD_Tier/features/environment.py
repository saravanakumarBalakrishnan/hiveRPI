...
from FF_testRailConnect import testRailConnect

BEHAVE_DEBUG_ON_ERROR = True
global testrailapiconnect
...
def before_all(context):
    ...
    istestrunupdate = oJsonDict['globalVariables']['istestrunupdate']

    if istestrunupdate == 'true':
        testrailurl = oJsonDict['globalVariables']['testrailurl']
        projectname = oJsonDict['globalVariables']['projectName']
        suitename = oJsonDict['globalVariables']['suiteName']
        testrailusername = oJsonDict['globalVariables']['testrailusername']
        testrailpassword = oJsonDict['globalVariables']['testrailpassword']
        testrunname = oJsonDict['globalVariables']['testrunname']
        testrailapi = testRailConnect()
        runid = testrailapi.addtestrun(projectname, suitename, testrailurl, testrailusername, testrailpassword,
                                       testrunname)
        # context.testrailapiconnect=testrailapiconnect

        reporter.testrailurl = testrailurl
        reporter.testrailusername = testrailusername
        reporter.testrailpassword = testrailpassword
        reporter.runid = runid
        reporter.currentcaseid = ""
        reporter.istestrunupdate = istestrunupdate
    ...
    if 'PLATFORM' in strAPIValidationType.upper():
        ...
        if strAPIValidationType == 'Zigbee API':
...
def login_client(context, strClient):
    ...
    strEnvironment = utils.getAttribute('common', 'currentEnvironment')
    if 'IOS' in strClient.upper():
        ...
        strDeviceLogger = utils.getAttribute('iOS', 'realDeviceLogger')
        strAutomationName = utils.getAttribute('iOS', 'automationName')
        context.iOSDriver = oBasePage.setup_ios_driver(strDeviceName, strAppPath, strDeviceLogger, strAutomationName)
        ...
        oLoginPage.select_environment(strEnvironment)
...
def before_scenario(context, scenario):
    ...
    if scenario.name.find("-- @") > 0:
        reporter.strCurrentScenario = scenario.name.split("-- @")[0].strip()
    else:
        reporter.strCurrentScenario = scenario.name
    ...
    if scenario.keyword == 'Scenario Outline':
        ...
        strCurrentScenarioDesc = scenario.name
    ...
    if cntr == '1':
        ...
        if scenario.name.find('_') > 0:
            ...
            reporter.strCurrentScenarioID = scenario.name

        if len(scenario.tags) > 1:
            currentcaseid = scenario.tags[0]
            n = 1
            currentcaseid = currentcaseid[n:]
            context.currentcaseid = currentcaseid
            reporter.currentcaseid = currentcaseid
    ...
    for oStep in scenario.steps:
        ...
        if oStep.keyword.upper() == 'GIVEN':
            for oStepWord in strStepName.split(' '):
                if oStepWord.upper() == 'API': strStepName = strStepName.replace(oStepWord,
                                                                                 '<B>' + utils.getAttribute('common',
                                                                                                            'apiValidationType') + '</B>')
            ...
            for oStepWord in strStepName.split(' '):
                ...
                if oStepWord.upper() in clientDict.keys():
                    ...
                    if not clientDict[oStepWord.upper()] in oScenarioClientsDict.keys(): oScenarioClientsDict[
                        clientDict[oStepWord.upper()]] = clientDict[oStepWord.upper()].split(' ')[0].upper()
...
#             print("GETDEVICE",resp)
                ...
                if not 'supportsHotWater' in oNode['attributes']:
                    if 'hardwareVersion' in oNode['attributes']:
                        ...
                        if intHardwareVersion == '00':
                            ...
                            if 'reportedValue' in oNode['attributes']['model']: strModel = oNode['attributes']['model'][
                                'reportedValue']
    ...
    print("DeviceversioDict*****",oDeviceVersionDict)
