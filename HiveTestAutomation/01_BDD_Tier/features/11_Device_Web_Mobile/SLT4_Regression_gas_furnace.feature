...
# Wiring configurations for this test W1, Y1, Rh, G, C, Acc
# The device mode should be selected to GAS FURNACE
# Created by bharath.gopalan at 27/09/2017

Feature: Regression cases for SLT4 stat in gas furnace

    @STH_US_RegressionTest @STH_REG_GAS_FURNACE_001
      Scenario Outline: STH_REG_GAS_FURNACE_001_verify in the gas furnace mode , Fan turns on when there is a cooling demand
      Given the TGstick is paried with the SLT4 thermostat and the device is switched to  <temp_scale> scale
      When the device is in <device_mode> mode and the required temperature is set to <number> points below the ambient temperature
      Then The device fan should be turned on

      Examples: Device Valid mode transtions

      |temp_scale|device_mode|number|
      |FAHRENHEIT|COOL HOLD  |5     |
      |CELCIUS   |COOL HOLD  |5     |
      |FAHRENHEIT|COOL_BOOST |5     |
      |CELCIUS   |COOL_BOOST |5     |

      @STH_US_RegressionTest @STH_REG_GAS_FURNACE_002
      Scenario Outline: STH_REG_GAS_FURNACE_002_verify in the gas furnace mode , The fan should not be turned when there is a heating demand
      Given the TGstick is paried with the SLT4 thermostat and the device is switched to  <temp_scale> scale
      When the device is in <device_mode> mode and the required temperature is set to <number> points above the ambient temperature
      Then The device fan should be turned off

      Examples: Device Valid mode transtions

      |temp_scale|device_mode|number|
      |FAHRENHEIT|HEAT HOLD  |5     |
      |CELCIUS   |HEAT HOLD  |5     |
      |FAHRENHEIT|HEAT_BOOST |5     |
      |CELCIUS   |HEAT_BOOST |5     |


    @STH_US_RegressionTest @STH_REG_GAS_FURNACE_003
      Scenario Outline: STH_REG_GAS_FURNACE_003_verify Check if the user is able to set valaction mode
      Given the TGstick is paried with the SLT4 thermostat and the device is switched to  <temp_scale> scale
      When The vacation mode is set for 3 minutes from the current stat time
    ...
    @STH_US_RegressionTest @STH_REG_GAS_FURNACE_004
      Scenario Outline: STH_REG_GAS_FURNACE_004_verify Check if the user is able to Cancel the valaction mode
      Given the TGstick is paried with the SLT4 thermostat and the device is switched to  <temp_scale> scale
      When The vacation mode is set for 3 minutes from the current stat time
    ...
    @STH_US_RegressionTest @STH_REG_GAS_FURNACE_005
      Scenario Outline: STH_REG_GAS_FURNACE_005_verify if the vacation mode override the current settings and cancel any quick heating and cooling
      Given the TGstick is paried with the SLT4 thermostat and the device is switched to  <temp_scale> scale
      ...
      When The vacation mode is set for 3 minutes from the current stat time
      Then it should be switched to mode OFF
      Then The vacation mode must be enabled
    ...
    @STH_US_RegressionTest @STH_REG_GAS_FURNACE_006
      Scenario Outline: STH_REG_GAS_FURNACE_006_verify if the vacation mode has ended , It should restore the previous stat state (which was previously in dual hold mode)
        Given the TGstick is paried with the SLT4 thermostat and the device is switched to  <temp_scale> scale
        When the device is set to dual mode <mode> , with heating set to <heat_temp> and cooling set to <cool_temp>
        When The vacation mode is set for 3 minutes from the current stat time
        Then it should be switched to mode OFF
        When The holiday mode ends after 3 minutes
        Then it should be switched to mode <mode>
      ...
      @STH_US_RegressionTest @STH_REG_GAS_FURNACE_007
      Scenario Outline: STH_REG_GAS_FURNACE_007_verify if the vacation mode has ended , It should restore the previous stat state (which was previously in Heat hold mode)
        Given the TGstick is paried with the SLT4 thermostat and the device is switched to  <temp_scale> scale
      ...
      @STH_US_RegressionTest @STH_REG_GAS_FURNACE_008
      Scenario Outline: STH_REG_GAS_FURNACE_008_verify if the vacation mode has ended , It should restore the previous stat state (which was previously in Cool hold mode)
        Given the TGstick is paried with the SLT4 thermostat and the device is switched to  <temp_scale> scale
        When the device is in COOL HOLD mode and the required temperature is set to 5 points below the ambient temperature
        When The vacation mode is set for 3 minutes from the current stat time
        Then it should be switched to mode OFF
        When The holiday mode ends after 3 minutes
        Then it should be switched to mode COOL HOLD
        Then The heat temperature should be changed by 5 points
  ...
  @STH_US_RegressionTest @STH_REG_GAS_FURNACE_009
    Scenario Outline: STH_REG_GAS_FURNACE_009_verify if the user is able to switch to different modes
      Given the TGstick is paried with the SLT4 thermostat and the device is switched to  <temp_scale> scale
  ...
  @STH_US_RegressionTest @STH_REG_GAS_FURNACE_010
  Scenario Outline: STH_REG_GAS_FURNACE_010_verify if the stat is able to switch between different temperature Scale
    Given the TGstick is paried with the SLT4 thermostat and the device is switched to  <temp_scale> scale
  ...
  @STH_US_RegressionTest @STH_REG_GAS_FURNACE_011
    Scenario Outline: STH_REG_GAS_FURNACE_011_verify if the stat is able to be set to quick cool and heat mode
    Given the TGstick is paried with the SLT4 thermostat and the device is switched to  <temp_scale> scale
    When the device is in <device_mode> mode and the required temperature is set to <number> points <value> the ambient temperature
  ...
  @STH_US_RegressionTest @STH_REG_GAS_FURNACE_0012
  Scenario Outline: STH_REG_GAS_FURNACE_0012 verify the override values in heat boost and heat hold mode
    Given the TGstick is paried with the SLT4 thermostat and the device is switched to  <temp_scale> scale
  ...
  @STH_US_RegressionTest @STH_REG_GAS_FURNACE_013
  Scenario Outline: STH_REG_GAS_FURNACE_013 verify the override values in cool boost and cool hold mode
  Given the TGstick is paried with the SLT4 thermostat and the device is switched to  <temp_scale> scale
  When the device is in <device_mode> mode and the required temperature is set to <number> points <value> the ambient temperature without protection timer validation
...
@STH_US_RegressionTest @STH_REG_GAS_FURNACE_014
  Scenario Outline: STH_REG_GAS_FURNACE_014_verify if the protection timer functionality
    Given the TGstick is paried with the SLT4 thermostat and the device is switched to  <temp_scale> scale
...
@STH_US_RegressionTest @STH_REG_GAS_FURNACE_015
  Scenario Outline: STH_REG_GAS_FURNACE_015_verify when the device is in dual mode , The temperature values can be set properly
  Given the TGstick is paried with the SLT4 thermostat and the device is switched to  <temp_scale> scale
  When the device is set to dual mode <mode> , with heating set to <heat_temp> and cooling set to <cool_temp>
  Then The heat temperature should be <heat_temp>
  Then The cool temperature should be <cool_temp>
...
@STH_US_RegressionTest @STH_REG_GAS_FURNACE_016
Scenario Outline: STH_REG_GAS_FURNACE_016_verify if the device is able to toggle from Quick heat to Quick cool mode
  Given the TGstick is paried with the SLT4 thermostat and the device is switched to  <temp_scale> scale
...
@STH_US_RegressionTest @STH_REG_GAS_FURNACE_017
      Scenario Outline: STH_REG_GAS_FURNACE_017_verify if the user is able to change the boost duration
      Given the TGstick is paried with the SLT4 thermostat and the device is switched to  <temp_scale> scale
...
@STH_US_RegressionTest @STH_REG_GAS_FURNACE_018
      Scenario Outline: STH_REG_GAS_FURNACE_018_validate the different device fan modes
      Given the TGstick is paried with the SLT4 thermostat and the device is switched to  <temp_scale> scale
...
@STH_US_RegressionTest @STH_REG_GAS_FURNACE_019
    Scenario Outline: STH_REG_GAS_FURNACE_019_validate verify if the device is able to toggle to different modes and back to the original mode without a set point
    Given the TGstick is paried with the SLT4 thermostat and the device is switched to  <temp_scale> scale
...
@STH_US_RegressionTest @STH_REG_GAS_FURNACE_020
    Scenario Outline: STH_REG_GAS_FURNACE_020_validate if the user is able to set the device name on the screen
    Given the TGstick is paried with the SLT4 thermostat and the device is switched to  <temp_scale> scale
    ...
    |CELCIUS   |test_device2|


@STH_US_RegressionTest @STH_REG_GAS_FURNACE_021
    Scenario Outline: STH_REG_GAS_FURNACE_021_validate When Quick Cool while already active on stat,the latest temperature should be overridden on stat
    Given the TGstick is paried with the SLT4 thermostat and the device is switched to  <temp_scale> scale
...
@STH_US_RegressionTest @STH_REG_GAS_FURNACE_022
    Scenario Outline: STH_REG_GAS_FURNACE_022_validate When Quick heat while already active on stat,the latest temperature should be overridden on stat
    Given the TGstick is paried with the SLT4 thermostat and the device is switched to  <temp_scale> scale
...
@STH_US_RegressionTest @STH_REG_GAS_FURNACE_023
    Scenario Outline: STH_REG_GAS_FURNACE_023_validate stat should return to the previous mode once quick heat/ cool is completed
    Given the TGstick is paried with the SLT4 thermostat and the device is switched to  <temp_scale> scale
...
@STH_US_RegressionTest @STH_REG_GAS_FURNACE_024
    Scenario Outline: SSTH_REG_GAS_FURNACE_024_validate stat should return to the previous dual hold when quick heat / cool is completed
    Given the TGstick is paried with the SLT4 thermostat and the device is switched to  <temp_scale> scale
...
@STH_US_RegressionTest @STH_REG_GAS_FURNACE_025
      Scenario Outline: SSTH_REG_GAS_FURNACE_025_validate stat should return to the previous dual schedule when quick heat / cool is completed
      Given the TGstick is paried with the SLT4 thermostat and the device is switched to  <temp_scale> scale
