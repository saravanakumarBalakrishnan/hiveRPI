Feature: Set the Bindings between Nodes and validate the same
  ...
  @SC-MSC-03 @ValidateAlarm
  Scenario: SC-MSC-03_Set the given Bindings between Nodes and validate the same
    Given The telegesis is paired with given devices
    When The zigbee message is sent by the device is read and validated infinitely for alarm message
