...
class BatchTrigger:
    ...
    def build_execute_kitlist(self):
        try:
            ...
            if len(oKitList.keys()) > 0:
                utils.setAttribute_KitBatch('batch_execution', 'current_batch_result_folder',
                ...
                for oPKey in sorted(oKitPriority.keys()):
                    ...
                    parallelExecThread = threading.Thread(target=self.trigger_parallel_kit_execution,
    ...
    def trigger_parallel_kit_execution(self, oKitPkey):
        ...
        self.update_main_result_json(oKitPkey, strResultFolder, User_ID, Test_Suite, Main_Client, Second_Client,
        ...
        for oKey in oDeviceVersionDict.keys():
            Kit_Setup = Kit_Setup + oDeviceVersionDict[oKey]['model'] + ' ==> ' + oDeviceVersionDict[oKey][
                'version'] + '<br>'
        self.Batch_Execution_Summary_KitAddLink(oKitPkey, strHTMLSummaryPath, Main_Client, Second_Client, User_ID,
    ...
    def update_gloabalVar_json(self, current_batch_id=None, current_kit_id=None, appium_node='4723', strDeviceID=""):
        ...
        if current_batch_id is None or current_kit_id is None: return
        ...
        if current_batch_id != "" and current_kit_id != "":
            ...
            if 'IOS' in oKitDetails['secondClient']['name'].upper():
                ...
                oCurrentEnvDict['android' + strAppVer]['platformVersion'] = oKitDetails['secondClient'][
                    'platformVersion']
            elif 'WEB' in oKitDetails['mainClient']['name'].upper():
            ...
            return strResultFolder, oGlobalDict['userName'], oGlobalDict['test_suite'], oGlobalDict['mainClient'], \
                   oGlobalDict['secondClient'], oGlobalDict['currentEnvironment']

    def update_main_result_json(self, oKitPkey, strResultFolder, User_ID, Test_Suite, Main_Client, Second_Client,
                                strCurrentEnvironment):
    ...
    def Batch_Execution_Summary_Initialize(self):
        ...
        if not strSystemResultFolderName == "":
            ...
            self.strResultsPath = os.path.abspath(__file__ + "/../../../") + '/03_Results_Tier/'
        strResultsPath = self.strResultsPath + utils.getAttribute_KitBatch('batch_execution',
                                                                           'current_batch_result_folder') + '/'
    ...
    def Batch_Execution_Summary_KitAddLink(self, Kit_ID, strHTMLSummaryPath, Main_Client, Second_Client, User_ID,
                                           Kit_Setup, Test_Suite):
        try:
            ...
            oFileWriter.write(
                "<td class='justified'><a href='" + './Kit_Results/' + strHTMLSummaryPath + "' target='about_blank'>" + str(
                    Kit_ID) + "</a></td>\n")
            ...
            oFileWriter.write("<td class='justified'>" + utils.getAttribute_KitBatch('batch_execution',
                                                                                     'current_kit_execution_time') + "</td>\n")
    ...
    def Batch_Execution_Summary_Footer(self):
        ...
        try:
            ...
            if not self.intPassKITCount == 0:
                oFileWriter.write("{  y: " + str(self.intPassKITCount) + ", legendText:'PASS', indexLabel: '{y}' },\n")
            else:
                oFileWriter.write("{  y: " + str(self.intPassKITCount) + ", legendText:'PASS'},\n")
            if not self.intFailKITCount == 0:
                oFileWriter.write("{  y: " + str(self.intFailKITCount) + ", legendText:'FAIL' , indexLabel: '{y}'}\n")
            else:
                oFileWriter.write("{  y: " + str(self.intFailKITCount) + ", legendText:'FAIL'}\n")
    ...
    def getDuration(self, strDuration):
        ...
        if intHour > 0:
            ...
            if intHour > 23:
                intDay = intHour // 24
                intHour = intHour % 24
                strDuration = str(intDay) + " day(s), " + str(intHour) + " hour(s), " + str(
