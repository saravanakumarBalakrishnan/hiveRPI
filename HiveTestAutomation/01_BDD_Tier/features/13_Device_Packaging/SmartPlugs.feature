Feature: Contains all the scenario related to Smart Plugs
  ...
  @SC-SP-DP01-04 @ZigbeeDumpTest
  Scenario Outline: SC-ZT-04_The smart plug state is changed and validated the using zigbee attribute and repeated infinitely.
    Given The telegesis is paired with given <DeviceType>
    When the <DeviceType> state is changed to below states and validated using the zigbee attribute and repeated infinitely
      | State |
      | ON    |
      | OFF   |
