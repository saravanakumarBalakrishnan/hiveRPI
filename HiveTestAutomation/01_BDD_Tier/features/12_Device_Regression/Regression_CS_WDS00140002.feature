...
Feature: Performs regression on the Motion Sensor for the Device type WDS00140002
  ...
  @WDS00140002_RegressionTest @SC-ZT-29
  Scenario Outline: SC-ZT-29_Download the zigbee dump from the device and validate with the baseline dump file via telegesis stick.
  ...
  @WDS00140002_RegressionTest @SC-NT-JN05-20
  Scenario: SC-NT-JN05-20_The given devices are paired and unpaired sequentially and validated
    Given The telegesis is paired with given devices
    When the below devices are paired and unpaired sequentially on all Zigbee Channels and validated via Telegesis
      ...
      | MS         | WDS00140002 | 00124B000D7C0318 |

  @WDS00140002_RegressionTest @SC-GT-SC06-11
  Scenario: SC-GT-SC06-11_The given devices are paired and unpaired sequentially and validated in the same channel twice
    Given The telegesis is paired with given devices
  ...
  @WDS00140002_RegressionTest @SC-FT-59
  Scenario: SC-FT-59_Downgrade firmware of the device and later upgrade the firmware to the previous version
    Given The telegesis is paired with given devices
    When the below list of DeviceName of DeviceType is upgraded and downgraded between DeviceVersion1 and DeviceVersion2 via Telegisis
      ...
      | MS         | WDS00140002 | NA             | 0.3.9          |
      | MS         | WDS00140002 | 0.4.0          | NA             |
      | MS         | WDS00140002 | NA             | 0.3.9          |

  @WDS00140002_RegressionTest @SC-BT-35
  Scenario Outline: SC-BT-35_To set bindings and validate the binding table on the device via telegesis stick.
    Given The telegesis is paired with given devices
    When the zigbee dump for the <DeviceType> is downloaded with the clusters and attributes list via telegesis stick
