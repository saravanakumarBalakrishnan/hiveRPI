# Created by fazila.nizar at 11/05/2017
  ...
  Feature: Performs regression on Active Light for the Device type TWBulb01US
  ...
  @TWBulb01US_RegressionTest @SC-RG-ZT-22 @ZigbeeDumpTest
  Scenario Outline: SC-RG-ZT-22_Download the zigbee dump from the device and validate with the baseline dump file via telegesis stick.
  ...
  @TWBulb01US_RegressionTest @SC-RG-NT-JN01-12 @PairingTest
  Scenario: SC-RG-NT-JN01-12_The given devices are paired and unpaired sequentially and validated once in all Zigbee Channels
    Given The telegesis is paired with given devices
    When the below devices are paired and unpaired sequentially on all Zigbee Channels and validated via Telegesis
      ...
      | AL         | TWBulb01US | 00158D00012E4D30 |

  @TWBulb01US_RegressionTest @SC-NT-JN05-12 @Generic
  ...
  @TWBulb01US_RegressionTest @SC-FT-53 @FirmwareTest
  Scenario: SC-FT-53_Downgrade firmware of the device and later upgrade the firmware to the previous version
    Given The telegesis is paired with given devices
    When the below list of DeviceName of DeviceType is upgraded and downgraded between DeviceVersion1 and DeviceVersion2 via Telegisis
      ...
      | AL         | TWBulb01US | NA             | 11160002       |
      | AL         | TWBulb01US | 11200002       | NA             |

  @TWBulb01US_RegressionTest @SC-SP-DP01-12 @DevicePairingTest
  Scenario Outline: SC-SP-DP01-12_The Device state is changed and validated the using zigbee attribute twice
  ...
  @TWBulb01US_RegressionTest @SC-TW-02 @ActiveLight
    Scenario: SC-TW-02_Change Tune and Brightness in a TW Bulb by changing the Tune and Brightness respectively
    Given The telegesis is paired with given devices
  ...
  @TWBulb01US_RegressionTest @SC-BT-28 @ZigbeeBindingTest
  Scenario Outline: SC-BT-28_To set bindings and validate the binding table on the device via telegesis stick.
    Given The telegesis is paired with given devices
    When the zigbee dump for the <DeviceType> is downloaded with the clusters and attributes list via telegesis stick
