...
Feature: Performs regression on Active Light for the Device type FWBulb01US
...
#Zigbee Binding Test
  ...
  @FWBulb01US_RegressionTest @SC-RG-ZT-24 @ZigbeeDumpTest
  Scenario Outline: SC-RG-ZT-24_Download the zigbee dump from the device and validate with the baseline dump file via telegesis stick.
  ...
  @FWBulb01US_RegressionTest @SC-RG-NT-JN01-14 @PairingTest
  Scenario: SC-RG-NT-JN01-14_The given devices are paired and unpaired sequentially and validated once in all Zigbee Channels
    Given The telegesis is paired with given devices
    When the below devices are paired and unpaired sequentially on all Zigbee Channels and validated via Telegesis
      ...
      | AL         | FWBulb01US | 00158D00013BBDCA |

  @FWBulb01US_RegressionTest @SC-NT-JN05-14 @Generic
  Scenario: SC-NT-JN05-14_The given devices are paired and unpaired sequentially and validated in the same channel twice
    Given The telegesis is paired with given devices
    When the below devices are paired and unpaired sequentially and validated 2 times via Telegesis
      | DeviceName | DeviceType | MacID            |
      | AL         | FWBulb01US | 00158D00013BBDCA |

  @FWBulb01US_RegressionTest @SC-FT-55 @FirmwareTest
  Scenario: SC-FT-55_Downgrade firmware of the device and later upgrade the firmware to the previous version
    Given The telegesis is paired with given devices
    When the below list of DeviceName of DeviceType is upgraded and downgraded between DeviceVersion1 and DeviceVersion2 via Telegisis
      ...
      | AL         | FWBulb01US | NA             | 11300002       |
      | AL         | FWBulb01US | 11340002       | NA             |

  @FWBulb01US_RegressionTest @SC-SP-DP01-14 @DevicePairingTest
  Scenario Outline: SC-SP-DP01-14_The Device state is changed and validated the using zigbee attribute twice
  ...
  @FFWBulb01US_RegressionTest @SC-FW-02 @ActiveLight
  Scenario: SC-FW-02_Switch ActiveLight to change the brightness of the light and validate the same
    Given The telegesis is paired with given devices
  ...
  @FWBulb01US_RegressionTest @SC-BT-30 @ZigbeeBindingTest
  Scenario Outline: SC-BT-30_To set bindings and validate the binding table on the device via telegesis stick.
    Given The telegesis is paired with given devices
    When the zigbee dump for the <DeviceType> is downloaded with the clusters and attributes list via telegesis stick
