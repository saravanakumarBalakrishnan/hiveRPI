Feature: Contains all the Regression scenarios for Smart Plugs testing
  ...
  Scenario: SC-SP-FT02-01_Downgrade  firmware of the device and later upgrade the firmware to the previous version for the given list of devices infinitely
    ...
    @SLP2d_Regression @SP-OnOFF-08 @OnOFF
  Scenario Outline: SP-OnOFF-08_The smart plug state is changed and validated the using zigbee attribute twice
    Given The telegesis is paired with given <DeviceType>
    When the <DeviceType> state is changed for 1200 to below states and validated using the zigbee attribute and repeated 6 times
      | State |
      | OFF   |
      | ON    |
