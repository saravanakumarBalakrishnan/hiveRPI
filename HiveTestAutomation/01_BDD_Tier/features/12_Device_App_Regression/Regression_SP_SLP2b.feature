...
Feature: Performs regression on Smart Plug for the Device type SLP2b
  ...
  @SC-NT-JN04-03 @Regression_API_SLP2b_Test
  Scenario: SC-NT-JN04-03_The given devices are paired and unpaired sequentially and validated
    ...
    @SC-SP-OO-01 @Regression_API_SLP2b_Test
  Scenario: SC-AL-01_Switch Plug to ON and OFF state and change the brightness of the light and validate the same
	Given The Hive product is paired and setup for PLUG
	When The SLP2b_1 is switched ON and validated via Hub
    Then Automatically validate current state as ON while in Manual mode
	When The SLP2b_1 is switched OFF and validated via Hub
    Then Automatically validate current state as OFF while in Manual mode



  @SC-SP-SH12-02 @Regression_API_SLP2b_Test
  ...
  @SC-FT-14-37 @FirmwareTest @Regression_API_SLP2b_Test
  Scenario: SC-FT-04-37_Downgrade  firmware of the device and later upgrade the firmware to the previous version for the given list of devices once
    ...
    When the below list of DeviceName of DeviceType is upgraded and downgraded between DeviceVersion1 and DeviceVersion2 and validated via HUB
      ...
      | SP         | SLP2b      | 01035700       | NA             |
      | SP         | SLP2b      | NA             | 01045700       |

  @SC-OFF-RS10 @OfflineTest
  Scenario: SC-OFF-RS10_Validate the the time taken for devices to come Online after Hub Reboot
    Given The Hive product is paired and setup for PLUG
