...
class UIClass(Frame):
    def __init__(self, parent):
        ...
        ttk.Label(self.mainframe, text='Client Test Automation', font=self.appHighlightFont).grid(column=15, row=0,
                                                                                                  sticky=W)
        ...
        self.oMainClinetRD = Radiobutton(self.mainframe, text='Main Client', padx=3,
                                         variable=self.SelectedClientRadioButton, value='Main Client',
                                         command=self.set_main_or_second_client)
        self.oMainClinetRD.grid(column=3, row=3, sticky=E)
        self.oSecondClinetRD = Radiobutton(self.mainframe, text='Second Client', padx=3,
                                           variable=self.SelectedClientRadioButton, value='Second Client',
                                           command=self.set_main_or_second_client)
        self.oSecondClinetRD.grid(column=15, row=3, sticky=W)
        self.oMainClinetRD.select()
        self.SecondClientFlag = StringVar()
        self.oSecondClientFlag = Checkbutton(self.mainframe, variable=self.SecondClientFlag,
                                             command=self.set_second_client_flag)
        ...
        '''
        oTestSuiteList = (
            'BasicSmokeTest_Dual', 'BasicSmokeTest_Heating', 'BasicSmokeTest_HotWater', 'ScheduleTest_Dual',
            'ScheduleTest_Heating', 'ScheduleTest_HotWater', 'Test_Batch')
        ...
        self.oProgressbar = ttk.Progressbar(self.executebuttonFrame, orient="horizontal", length=200,
                                            mode="indeterminate")
        ...
        self.oBatchRemoveKit = Button(self.batchlistbuttonFrame, image=self.oRemoveIMG1,
                                      command=self.remove_batch_from_list)
    ...
    def load_kit_details(self, event):
        ...
        if current_batch_id != "" and current_kit_id != "":
            ...
            ALAPI.createCredentials(oKitDetails['currentEnvironment'], username=oKitDetails['userName'],
                                    password=oKitDetails['password'])
    ...
    def rename_batch(self):
        ...
        while boolNameAlreadyExists:
            ...
            if current_batch_id.upper() == 'TEMP_BATCH':
                ...
                return False
            strNewBatchName = simpledialog.askstring('Batch - New Name', 'Enter the Name of the Batch',
                                                     parent=self.window, initialvalue=current_batch_id)
        ...
        oJsonDict = self.get_kit_batch_json()
        oJsonDict["kit_batch"]['list_of_batches'][strNewBatchName] = oJsonDict["kit_batch"]['list_of_batches'].pop(
            current_batch_id)
    ...
    def rename_kit(self):
        ...
        while boolNameAlreadyExists:
            ...
            strNewKitName = simpledialog.askstring('Kit - New Name', 'Enter the Name of the Kit', parent=self.window,
                                                   initialvalue=current_kit_id)
            ...
            for strKitName in self.oKitID.get(0, END):
                ...
                intIndex = intIndex + 1
        oJsonDict = self.get_kit_batch_json()
        oJsonDict["kit_batch"]['list_of_batches'][current_batch_id]['list_of_kits'][strNewKitName] = \
            oJsonDict["kit_batch"]['list_of_batches'][current_batch_id]['list_of_kits'].pop(current_kit_id)
        self.put_kit_batch_json(oJsonDict)
        self.load_kits_to_list(None, False)
    ...
    def save_as_batch(self):
        ...
        oJsonDict = self.get_kit_batch_json()
        oJsonDict["kit_batch"]['list_of_batches'][strTimeStamp] = oJsonDict["kit_batch"]['list_of_batches'][
            current_batch_id]
        self.put_kit_batch_json(oJsonDict)
    ...
    def remove_batch_from_list(self):
        ...
        current_batch_id, current_kit_id = self.get_selected_batch_kit()
        strClickedButton = askquestion('Remove Batch',
                                       'Are you sure you want to remove the selected Batch: ' + current_batch_id,
    ...
    def add_new_kit(self):
        ...
        if self.session.statusCode != 200:
            ...
            oJsonDict["kit_batch"]['list_of_batches'][current_batch_id]['list_of_kits'][strTimeStamp] = {
                "priority": str(intPiority)}
            self.put_kit_batch_json(oJsonDict)
    ...
    def update_kit_details(self, boolValidateLoginCred=True):
        ...
        if not (current_batch_id == "" and current_kit_id == ""):
            if boolValidateLoginCred:
                ALAPI.createCredentials(self.Environment.get(), username=self.UserName.get(),
            ...
            oJsonDict = self.get_kit_batch_json()
            intCurrentKitPriority = int(
                oJsonDict["kit_batch"]['list_of_batches'][current_batch_id]['list_of_kits'][current_kit_id]['priority'])
            oKitDetails = oJsonDict["kit_batch"]['list_of_batches'][current_batch_id]['list_of_kits'][current_kit_id]
            ...
            if self.SecondClientFlag.get() == '1':
                oKitDetails['secondClientValidateFlag'] = 'YES'
            else:
                oKitDetails['secondClientValidateFlag'] = 'NO'
    ...
    def move_up_priority(self):
        ...
        if len(current_batch_id) > 0:
            ...
            if len(current_kit_id) > 0:
                ...
                intCurrentKitPriority = int(
                    oJsonDict["kit_batch"]['list_of_batches'][current_batch_id]['list_of_kits'][current_kit_id][
                        'priority'])
                ...
                for oKit in oKitList:
                    ...
                    if intPriority == intSetPriorityTo:
                        oJsonDict["kit_batch"]['list_of_batches'][current_batch_id]['list_of_kits'][oKit][
                            'priority'] = str(intCurrentKitPriority)
                oJsonDict["kit_batch"]['list_of_batches'][current_batch_id]['list_of_kits'][current_kit_id][
                    'priority'] = str(intSetPriorityTo)
                self.put_kit_batch_json(oJsonDict)
                self.load_kits_to_list(None, False)
    ...
    def remove_kit_from_list(self):
        ...
        if len(current_batch_id) > 0:
            ...
            if len(current_kit_id) > 0:
                ...
                current_kit_id = self.oKitID.get(current_kit_id[0])
                strClickedButton = askquestion('Remove Kit',
                                               'Are you sure you want to remove the selected Kit: ' + current_kit_id,
                                               parent=self, default=NO, icon=WARNING)
    ...
    def load_app_details(self, event):
        ...
        if 'WEB' in self.MainClient.get().upper():
            ...
            if self.boolWebFieldDestroyed:
                # Browser Name
                self.lblBrowserName, self.oBrowserName = self.create_combobox(5, '     Browser Name:    ',
                                                                              self.BrowserName, (), "", False)
            ...
            if self.boolAppFieldDestroyed:
                # AppFilePath
                self.lblAppFileName, self.oAppFilePath = self.create_combobox(5, 'App File Name:    ', self.AppFilePath,
                                                                              (), "", False)
                ...
                # OS platform version
                self.lblOSVersion, self.oOSVersion = self.create_combobox(6, 'Mobile OS Version:    ', self.OSVersion,
                                                                          (), "", False)
                # Device Name
                self.lblDeviceName, self.oDeviceName = self.create_combobox(7, 'Device Name:    ', self.DeviceName, (),
                                                                            "", False)
    ...
    def load_app_filename(self, strFilePath):
        if self.MainClient.get().split()[0] == 'Android':
            oAppExt = ("APK", "*.apk")
        else:
            oAppExt = ("IPA", "*.ipa")
        strAPPFolderPAth = os.path.abspath(__file__ + "/../../../") + "/02_Manager_Tier/EnviromentFile/Apps/" + \
                           self.MainClient.get().split()[0] + '/' + self.Environment.get().split()[0]
    ...
    def updateGlobalVarJson(self):
        ...
        strClickedButton = askquestion('Execute', 'Are you sure you want to Execute the selected Batch : ' +
                                       self.get_selected_batch_kit()[0], parent=self, default=YES, icon=WARNING)
    ...
    def trigger_test_run(self):
        ...
        if strTestSuite == 'BasicSmokeTest_Dual':
            ...
            oProcess = subprocess.Popen("behave --tags=BasicSmokeTest --tags=Heating", stdout=subprocess.PIPE,
                                        shell=True)
        elif strTestSuite == 'BasicSmokeTest_HotWater':
            oProcess = subprocess.Popen("behave --tags=BasicSmokeTest --tags=HotWater", stdout=subprocess.PIPE,
                                        shell=True)
            ...
            oProcess = subprocess.Popen("behave --tags=ScheduleTest --tags=Verify --tags=Heating",
                                        stdout=subprocess.PIPE, shell=True)
        elif strTestSuite == 'ScheduleTest_HotWater':
            oProcess = subprocess.Popen("behave --tags=ScheduleTest --tags=Verify --tags=HotWater",
                                        stdout=subprocess.PIPE, shell=True)
    ...
    def update_gloabalVar_json(self, current_batch_id=None, current_kit_id=None):
        ...
        if current_batch_id is None or current_kit_id is None: current_batch_id, current_kit_id = self.get_selected_batch_kit()
    ...
    def browse_app_file(self):
        ...
        strAPPFolderPAth = os.path.abspath(__file__ + "/../../../") + "/02_Manager_Tier/EnviromentFile/Apps/" + \
                           self.MainClient.get().split()[0] + '/' + self.Environment.get().split()[0]
        strFilePath = askopenfilename(filetypes=[oAppExt], initialdir=strAPPFolderPAth,
                                      title="Select the Android APK file")
        strFileName = ntpath.split(strFilePath)[1]
